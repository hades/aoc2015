...
struct poly_rewriter_params {
  params_ref const& p;
  params_ref g;
  poly_rewriter_params(params_ref const& _p = params_ref::get_empty()): p(_p), g(gparams::get_module("rewriter")) {}
  static void collect_param_descrs(param_descrs& d) {
    d.insert("som", CPK_BOOL, "put polynomials in sum-of-monomials form", "false", "rewriter");
    d.insert("som_blowup", CPK_UINT,
             "maximum increase of monomials generated when putting a polynomial in sum-of-monomials normal form", "10",
             "rewriter");
    d.insert("hoist_mul", CPK_BOOL, "hoist multiplication over summation to minimize number of multiplications",
             "false", "rewriter");
