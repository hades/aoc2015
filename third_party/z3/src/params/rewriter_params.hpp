...
struct rewriter_params {
  params_ref const& p;
  params_ref g;
  rewriter_params(params_ref const& _p = params_ref::get_empty()): p(_p), g(gparams::get_module("rewriter")) {}
  static void collect_param_descrs(param_descrs& d) {
    ...
    d.insert("bv_ineq_consistency_test_max", CPK_UINT,
             "max size of conjunctions on which to perform consistency test based on inequalities on bitvectors.", "0",
             "rewriter");
    ...
    d.insert("ignore_patterns_on_ground_qbody", CPK_BOOL,
             "ignores patterns on quantifiers that don't mention their bound variables.", "true", "rewriter");
  }
  /*
     REG_MODULE_PARAMS('rewriter', 'rewriter_params::collect_param_descrs')
     REG_MODULE_DESCRIPTION('rewriter', 'new formula simplification module used in the tactic framework, and new
     solvers')
  */
