...
struct pattern_inference_params_helper {
  ...
  static void collect_param_descrs(param_descrs& d) {
    d.insert("max_multi_patterns", CPK_UINT,
             "when patterns are not provided, the prover uses a heuristic to infer them, this option sets the "
             "threshold on the number of extra multi-patterns that can be created; by default, the prover creates at "
             "most one multi-pattern when there is no unary pattern",
             "0", "pi");
    ...
    d.insert("arith", CPK_UINT,
             "0 - do not infer patterns with arithmetic terms, 1 - use patterns with arithmetic terms if there is no "
             "other pattern, 2 - always use patterns with arithmetic terms",
             "1", "pi");
    ...
    d.insert("arith_weight", CPK_UINT,
             "default weight for quantifiers where the only available pattern has nested arithmetic terms", "5", "pi");
    d.insert("non_nested_arith_weight", CPK_UINT,
             "default weight for quantifiers where the only available pattern has non nested arithmetic terms", "10",
             "pi");
