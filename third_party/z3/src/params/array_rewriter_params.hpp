...
struct array_rewriter_params {
  params_ref const& p;
  params_ref g;
  array_rewriter_params(params_ref const& _p = params_ref::get_empty()): p(_p), g(gparams::get_module("rewriter")) {}
  static void collect_param_descrs(param_descrs& d) {
    d.insert("expand_select_store", CPK_BOOL,
             "conservatively replace a (select (store ...) ...) term by an if-then-else term", "false", "rewriter");
    d.insert("blast_select_store", CPK_BOOL, "eagerly replace all (select (store ..) ..) term by an if-then-else term",
             "false", "rewriter");
