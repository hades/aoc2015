...
struct arith_rewriter_params {
  params_ref const& p;
  params_ref g;
  arith_rewriter_params(params_ref const& _p = params_ref::get_empty()): p(_p), g(gparams::get_module("rewriter")) {}
  static void collect_param_descrs(param_descrs& d) {
    d.insert("algebraic_number_evaluator", CPK_BOOL,
             "simplify/evaluate expressions containing (algebraic) irrational numbers.", "true", "rewriter");
    d.insert("mul_to_power", CPK_BOOL, "collpase (* t ... t) into (^ t k), it is ignored if expand_power is true.",
             "false", "rewriter");
    ...
    d.insert("max_degree", CPK_UINT, "max degree of algebraic numbers (and power operators) processed by simplifier.",
             "64", "rewriter");
    ...
    d.insert("arith_lhs", CPK_BOOL,
             "all monomials are moved to the left-hand-side, and the right-hand-side is just a constant.", "false",
             "rewriter");
    d.insert("arith_ineq_lhs", CPK_BOOL, "rewrite inequalities so that right-hand-side is a constant.", "false",
             "rewriter");
    d.insert("elim_to_real", CPK_BOOL, "eliminate to_real from arithmetic predicates that contain only integers.",
             "false", "rewriter");
