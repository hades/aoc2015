...
struct solver_params {
  params_ref const& p;
  params_ref g;
  solver_params(params_ref const& _p = params_ref::get_empty()): p(_p), g(gparams::get_module("solver")) {}
  static void collect_param_descrs(param_descrs& d) {
    ...
    d.insert("axioms2files", CPK_BOOL, "print negated theory axioms to separate files during search", "false",
             "solver");
    ...
    d.insert("proof.save", CPK_BOOL, "save proof log into a proof object that can be extracted using (get-proof)",
             "false", "solver");
    d.insert("proof.trim", CPK_BOOL, "trim and save proof into a proof object that an be extracted using (get-proof)",
             "false", "solver");
  }
  /*
