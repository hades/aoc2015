...
struct bool_rewriter_params {
  params_ref const& p;
  params_ref g;
  bool_rewriter_params(params_ref const& _p = params_ref::get_empty()): p(_p), g(gparams::get_module("rewriter")) {}
  static void collect_param_descrs(param_descrs& d) {
    d.insert(
        "ite_extra_rules", CPK_BOOL,
        "extra ite simplifications, these additional simplifications may reduce size locally but increase globally",
        "true", "rewriter");
    ...
    d.insert("blast_distinct", CPK_BOOL, "expand a distinct predicate into a quadratic number of disequalities",
             "false", "rewriter");
    d.insert(
        "blast_distinct_threshold", CPK_UINT,
