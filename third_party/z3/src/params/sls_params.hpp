...
struct sls_params {
  params_ref const& p;
  params_ref g;
  sls_params(params_ref const& _p = params_ref::get_empty()): p(_p), g(gparams::get_module("sls")) {}
  static void collect_param_descrs(param_descrs& d) {
    ...
    d.insert("walksat_ucb", CPK_BOOL, "use bandit heuristic for walksat assertion selection (instead of random)",
             "true", "sls");
    d.insert("walksat_ucb_constant", CPK_DOUBLE, "the ucb constant c in the term score + c * f(touched)", "20.0",
             "sls");
    d.insert("walksat_ucb_init", CPK_BOOL, "initialize total ucb touched to formula size", "false", "sls");
    d.insert("walksat_ucb_forget", CPK_DOUBLE, "scale touched by this factor every base restart interval", "1.0",
             "sls");
    d.insert("walksat_ucb_noise", CPK_DOUBLE, "add noise 0 <= 256 * ucb_noise to ucb score for assertion selection",
             "0.0002", "sls");
    ...
    d.insert("vns_mc", CPK_UINT, "in local minima, try Monte Carlo sampling vns_mc many 2-bit-flips per bit", "0",
             "sls");
    d.insert("vns_repick", CPK_BOOL, "in local minima, try picking a different assertion (only for walksat)", "false",
             "sls");
    ...
    d.insert("track_unsat", CPK_BOOL, "keep a list of unsat assertions as done in SAT - currently disabled internally",
             "false", "sls");
