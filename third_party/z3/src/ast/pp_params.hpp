...
struct pp_params {
  params_ref const& p;
  params_ref g;
  pp_params(params_ref const& _p = params_ref::get_empty()): p(_p), g(gparams::get_module("pp")) {}
  static void collect_param_descrs(param_descrs& d) {
    ...
    d.insert("min_alias_size", CPK_UINT,
             "min. size for creating an alias for a shared term (when pretty printing SMT2 terms/formulas)", "10",
             "pp");
    d.insert("decimal", CPK_BOOL,
             "pretty print real numbers using decimal notation (the output may be truncated). Z3 adds a ? if the value "
             "is not precise",
             "false", "pp");
    ...
    d.insert("bv_literals", CPK_BOOL, "use Bit-Vector literals (e.g, #x0F and #b0101) during pretty printing", "true",
             "pp");
    d.insert("fp_real_literals", CPK_BOOL,
             "use real-numbered floating point literals (e.g, +1.0p-1) during pretty printing", "false", "pp");
    d.insert("bv_neg", CPK_BOOL, "use bvneg when displaying Bit-Vector literals where the most significant bit is 1",
             "false", "pp");
