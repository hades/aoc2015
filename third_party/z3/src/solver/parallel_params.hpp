...
struct parallel_params {
  params_ref const& p;
  params_ref g;
  parallel_params(params_ref const& _p = params_ref::get_empty()): p(_p), g(gparams::get_module("parallel")) {}
  static void collect_param_descrs(param_descrs& d) {
    d.insert("enable", CPK_BOOL, "enable parallel solver by default on selected tactics (for QF_BV)", "false",
             "parallel");
    d.insert("threads.max", CPK_UINT, "caps maximal number of threads below the number of processors", "10000",
             "parallel");
    ...
    d.insert("conquer.backtrack_frequency", CPK_UINT, "frequency to apply core minimization during conquer", "10",
             "parallel");
    d.insert("simplify.exp", CPK_DOUBLE, "restart and inprocess max is multiplied by simplify.exp ^ depth", "1",
             "parallel");
    d.insert("simplify.max_conflicts", CPK_UINT, "maximal number of conflicts during simplification phase",
             "4294967295", "parallel");
    d.insert("simplify.restart.max", CPK_UINT, "maximal number of restarts during simplification phase", "5000",
             "parallel");
    d.insert("simplify.inprocess.max", CPK_UINT, "maximal number of inprocessing steps during simplification", "2",
             "parallel");
  }
  /*
