...
struct fp_params {
  params_ref const& p;
  params_ref g;
  fp_params(params_ref const& _p = params_ref::get_empty()): p(_p), g(gparams::get_module("fp")) {}
  static void collect_param_descrs(param_descrs& d) {
    d.insert("engine", CPK_SYMBOL, "Select: auto-config, datalog, bmc, spacer", "auto-config", "fp");
    d.insert("datalog.default_table", CPK_SYMBOL,
             "default table implementation: sparse, hashtable, bitvector, interval", "sparse", "fp");
    d.insert("datalog.default_relation", CPK_SYMBOL, "default relation implementation: external_relation, pentagon",
             "pentagon", "fp");
    d.insert("datalog.generate_explanations", CPK_BOOL,
             "produce explanations for produced facts when using the datalog engine", "false", "fp");
    d.insert("datalog.use_map_names", CPK_BOOL, "use names from map files when displaying tuples", "true", "fp");
    d.insert("datalog.magic_sets_for_queries", CPK_BOOL, "magic set transformation will be used for queries", "false",
             "fp");
    d.insert("datalog.explanations_on_relation_level", CPK_BOOL,
             "if true, explanations are generated as history of each relation, rather than per fact "
             "(generate_explanations must be set to true for this option to have any effect)",
             "false", "fp");
    d.insert("datalog.unbound_compressor", CPK_BOOL,
             "auxiliary relations will be introduced to avoid unbound variables in rule heads", "true", "fp");
    d.insert("datalog.similarity_compressor", CPK_BOOL,
             "rules that differ only in values of constants will be merged into a single rule", "true", "fp");
    d.insert("datalog.similarity_compressor_threshold", CPK_UINT,
             "if similarity_compressor is on, this value determines how many similar rules there must be in order for "
             "them to be merged",
             "11", "fp");
    d.insert("datalog.all_or_nothing_deltas", CPK_BOOL,
             "compile rules so that it is enough for the delta relation in union and widening operations to determine "
             "only whether the updated relation was modified or not",
             "false", "fp");
    d.insert("datalog.compile_with_widening", CPK_BOOL, "widening will be used to compile recursive rules", "false",
             "fp");
    d.insert("datalog.default_table_checked", CPK_BOOL,
             "if true, the default table will be default_table inside a wrapper that checks that its results are the "
             "same as of default_table_checker table",
             "false", "fp");
    d.insert("datalog.default_table_checker", CPK_SYMBOL, "see default_table_checked", "null", "fp");
    d.insert("datalog.check_relation", CPK_SYMBOL,
             "name of default relation to check. operations on the default relation will be verified using SMT solving",
             "null", "fp");
    d.insert("datalog.initial_restart_timeout", CPK_UINT,
             "length of saturation run before the first restart (in ms), zero means no restarts", "0", "fp");
    d.insert("datalog.timeout", CPK_UINT, "Time limit used for saturation", "0", "fp");
    d.insert("datalog.output_profile", CPK_BOOL,
             "determines whether profile information should be output when outputting Datalog rules or instructions",
             "false", "fp");
    d.insert("datalog.print.tuples", CPK_BOOL, "determines whether tuples for output predicates should be output",
             "true", "fp");
    d.insert("datalog.profile_timeout_milliseconds", CPK_UINT,
             "instructions and rules that took less than the threshold will not be printed when printed the "
             "instruction/rule list",
             "0", "fp");
    d.insert("datalog.dbg_fpr_nonempty_relation_signature", CPK_BOOL,
             "if true, finite_product_relation will attempt to avoid creating inner relation with empty signature by "
             "putting in half of the table columns, if it would have been empty otherwise",
             "false", "fp");
    d.insert("datalog.subsumption", CPK_BOOL, "if true, removes/filters predicates with total transitions", "true",
             "fp");
    ...
    d.insert("spacer.simplify_lemmas_pre", CPK_BOOL, "simplify derived lemmas before inductive propagation", "false",
             "fp");
    d.insert("spacer.simplify_lemmas_post", CPK_BOOL, "simplify derived lemmas after inductive propagation", "false",
             "fp");
    d.insert("spacer.use_inductive_generalizer", CPK_BOOL, "generalize lemmas using induction strengthening", "true",
             "fp");
    d.insert("spacer.max_num_contexts", CPK_UINT, "maximal number of contexts to create", "500", "fp");
    d.insert("print_fixedpoint_extensions", CPK_BOOL,
             "use SMT-LIB2 fixedpoint extensions, instead of pure SMT2, when printing rules", "true", "fp");
    d.insert("print_low_level_smt2", CPK_BOOL,
             "use (faster) low-level SMT2 printer (the printer is scalable but the result may not be as readable)",
             "false", "fp");
    d.insert("print_with_variable_declarations", CPK_BOOL,
             "use variable declarations when displaying rules (instead of attempting to use original names)", "true",
             "fp");
    ...
    d.insert("print_boogie_certificate", CPK_BOOL,
             "print certificate for reachability or non-reachability using a format understood by Boogie", "false",
             "fp");
    ...
    d.insert("tab.selection", CPK_SYMBOL, "selection method for tabular strategy: weight (default), first, var-use",
             "weight", "fp");
    ...
    d.insert("xform.inline_linear_branch", CPK_BOOL, "try linear inlining method with potential expansion", "false",
             "fp");
    ...
    d.insert("xform.transform_arrays", CPK_BOOL, "Rewrites arrays equalities and applies select over store", "false",
             "fp");
    d.insert("xform.instantiate_arrays", CPK_BOOL, "Transforms P(a) into P(i, a[i] a)", "false", "fp");
    d.insert("xform.instantiate_arrays.enforce", CPK_BOOL, "Transforms P(a) into P(i, a[i]), discards a from predicate",
             "false", "fp");
    d.insert("xform.instantiate_arrays.nb_quantifier", CPK_UINT, "Gives the number of quantifiers per array", "1",
             "fp");
    d.insert("xform.instantiate_arrays.slice_technique", CPK_SYMBOL,
             "<no-slicing>=> GetId(i) = i, <smash> => GetId(i) = true", "no-slicing", "fp");
    d.insert("xform.quantify_arrays", CPK_BOOL, "create quantified Horn clauses from clauses with arrays", "false",
             "fp");
    d.insert("xform.instantiate_quantifiers", CPK_BOOL,
             "instantiate quantified Horn clauses using E-matching heuristic", "false", "fp");
    ...
    d.insert("xform.subsumption_checker", CPK_BOOL, "Enable subsumption checker (no support for model conversion)",
             "true", "fp");
    d.insert("xform.coi", CPK_BOOL, "use cone of influence simplification", "true", "fp");
    d.insert("spacer.order_children", CPK_UINT,
             "SPACER: order of enqueuing children in non-linear rules : 0 (original), 1 (reverse), 2 (random)", "0",
             "fp");
    d.insert("spacer.use_lemma_as_cti", CPK_BOOL, "SPACER: use a lemma instead of a CTI in flexible_trace", "false",
             "fp");
    d.insert("spacer.reset_pob_queue", CPK_BOOL, "SPACER: reset pob obligation queue when entering a new level", "true",
             "fp");
    d.insert("spacer.use_array_eq_generalizer", CPK_BOOL, "SPACER: attempt to generalize lemmas with array equalities",
             "true", "fp");
    d.insert("spacer.use_derivations", CPK_BOOL,
             "SPACER: using derivation mechanism to cache intermediate results for non-linear rules", "true", "fp");
    d.insert("xform.array_blast", CPK_BOOL,
             "try to eliminate local array terms using Ackermannization -- some array terms may remain", "false", "fp");
    ...
    d.insert("xform.elim_term_ite.inflation", CPK_UINT,
             "Maximum inflation for non-Boolean ite-terms blasting: 0 (none), k (multiplicative)", "3", "fp");
    ...
    d.insert("spacer.blast_term_ite_inflation", CPK_UINT,
             "Maximum inflation for non-Boolean ite-terms expansion: 0 (none), k (multiplicative)", "3", "fp");
    ...
    d.insert("spacer.iuc", CPK_UINT,
             "0 = use old implementation of unsat-core-generation, 1 = use new implementation of IUC generation, 2 = "
             "use new implementation of IUC + min-cut optimization",
             "1", "fp");
    d.insert("spacer.iuc.arith", CPK_UINT,
             "0 = use simple Farkas plugin, 1 = use simple Farkas plugin with constant from other partition (like old "
             "unsat-core-generation),2 = use Gaussian elimination optimization (broken), 3 = use additive IUC plugin",
             "1", "fp");
    d.insert("spacer.iuc.old_hyp_reducer", CPK_BOOL,
             "use old hyp reducer instead of new implementation, for debugging only", "false", "fp");
    ...
    d.insert("spacer.iuc.print_farkas_stats", CPK_BOOL,
             "prints for each proof how many Farkas lemmas it contains and how many of these participate in the cut "
             "(for debugging)",
             "false", "fp");
    d.insert("spacer.iuc.debug_proof", CPK_BOOL,
             "prints proof used by unsat-core-learner for debugging purposes (debugging)", "false", "fp");
    ...
    d.insert("spacer.use_lim_num_gen", CPK_BOOL, "Enable limit numbers generalizer to get smaller numbers", "false",
             "fp");
    d.insert("spacer.logic", CPK_SYMBOL, "SMT-LIB logic to configure internal SMT solvers", "", "fp");
    d.insert("spacer.arith.solver", CPK_UINT,
  ...
  */
  ...
  bool datalog_explanations_on_relation_level() const {
    return p.get_bool("datalog.explanations_on_relation_level", g, false);
  }
  ...
  unsigned datalog_similarity_compressor_threshold() const {
    return p.get_uint("datalog.similarity_compressor_threshold", g, 11u);
  }
  ...
  unsigned datalog_profile_timeout_milliseconds() const {
    return p.get_uint("datalog.profile_timeout_milliseconds", g, 0u);
  }
  bool datalog_dbg_fpr_nonempty_relation_signature() const {
    return p.get_bool("datalog.dbg_fpr_nonempty_relation_signature", g, false);
  }
  ...
  unsigned xform_instantiate_arrays_nb_quantifier() const {
    return p.get_uint("xform.instantiate_arrays.nb_quantifier", g, 1u);
  }
  symbol xform_instantiate_arrays_slice_technique() const {
    return p.get_sym("xform.instantiate_arrays.slice_technique", g, symbol("no-slicing"));
  }
