...
struct smt_params_helper {
  params_ref const& p;
  params_ref g;
  smt_params_helper(params_ref const& _p = params_ref::get_empty()): p(_p), g(gparams::get_module("smt")) {}
  static void collect_param_descrs(param_descrs& d) {
    ...
    d.insert("relevancy", CPK_UINT,
             "relevancy propagation heuristic: 0 - disabled, 1 - relevancy is tracked by only affects quantifier "
             "instantiation, 2 - relevancy is tracked, and an atom is only asserted if it is relevant",
             "2", "smt");
    d.insert("macro_finder", CPK_BOOL, "try to find universally quantified formulas that can be viewed as macros",
             "false", "smt");
    d.insert("quasi_macros", CPK_BOOL, "try to find universally quantified formulas that are quasi-macros", "false",
             "smt");
    d.insert("restricted_quasi_macros", CPK_BOOL,
             "try to find universally quantified formulas that are restricted quasi-macros", "false", "smt");
    d.insert("ematching", CPK_BOOL, "E-Matching based quantifier instantiation", "true", "smt");
    d.insert("phase_selection", CPK_UINT,
             "phase selection heuristic: 0 - always false, 1 - always true, 2 - phase caching, 3 - phase caching "
             "conservative, 4 - phase caching conservative 2, 5 - random, 6 - number of occurrences, 7 - theory",
             "3", "smt");
    ...
    d.insert("restart_strategy", CPK_UINT,
             "0 - geometric, 1 - inner-outer-geometric, 2 - luby, 3 - fixed, 4 - arithmetic", "1", "smt");
    d.insert("restart_factor", CPK_DOUBLE,
             "when using geometric (or inner-outer-geometric) progression of restarts, it specifies the constant used "
             "to multiply the current restart threshold",
             "1.1", "smt");
    d.insert("case_split", CPK_UINT,
             "0 - case split based on variable activity, 1 - similar to 0, but delay case splits created during the "
             "search, 2 - similar to 0, but cache the relevancy, 3 - case split based on relevancy (structural "
             "splitting), 4 - case split on relevancy and activity, 5 - case split on relevancy and current goal, 6 - "
             "activity-based case split with theory-aware branching activity",
             "1", "smt");
    d.insert("delay_units", CPK_BOOL, "if true then z3 will not restart when a unit clause is learned", "false", "smt");
    d.insert("delay_units_threshold", CPK_UINT,
             "maximum number of learned unit clauses before restarting, ignored if delay_units is false", "32", "smt");
    ...
    d.insert("candidate_models", CPK_BOOL,
             "create candidate models even when quantifier or theory reasoning is incomplete", "false", "smt");
    ...
    d.insert("threads.max_conflicts", CPK_UINT, "maximal number of conflicts between rounds of cubing for parallel SMT",
             "400", "smt");
    ...
    d.insert("mbqi.max_cexs", CPK_UINT,
             "initial maximal number of counterexamples used in MBQI, each counterexample generates a quantifier "
             "instantiation",
             "1", "smt");
    d.insert("mbqi.max_cexs_incr", CPK_UINT,
             "increment for MBQI_MAX_CEXS, the increment is performed after each round of MBQI", "0", "smt");
    d.insert("mbqi.max_iterations", CPK_UINT, "maximum number of rounds of MBQI", "1000", "smt");
    d.insert("mbqi.trace", CPK_BOOL,
             "generate tracing messages for Model Based Quantifier Instantiation (MBQI). It will display a message "
             "before every round of MBQI, and the quantifiers that were not satisfied",
             "false", "smt");
    d.insert("mbqi.force_template", CPK_UINT,
             "some quantifiers can be used as templates for building interpretations for functions. Z3 uses heuristics "
             "to decide whether a quantifier will be used as a template or not. Quantifiers with weight >= "
             "mbqi.force_template are forced to be used as a template",
             "10", "smt");
    d.insert("mbqi.id", CPK_STRING,
             "Only use model-based instantiation for quantifiers with id's beginning with string", "", "smt");
    d.insert("q.lift_ite", CPK_UINT,
             "0 - don not lift non-ground if-then-else, 1 - use conservative ite lifting, 2 - use full lifting of "
             "if-then-else under quantifiers",
             "0", "smt");
    ...
    d.insert("qi.cost", CPK_STRING, "expression specifying what is the cost of a given quantifier instantiation",
             "(+ weight generation)", "smt");
    d.insert("qi.max_multi_patterns", CPK_UINT, "specify the number of extra multi patterns", "0", "smt");
    d.insert("qi.quick_checker", CPK_UINT,
             "specify quick checker mode, 0 - no quick checker, 1 - using unsat instances, 2 - using both unsat and "
             "no-sat instances",
             "0", "smt");
    ...
    d.insert("bv.size_reduce", CPK_BOOL,
             "pre-processing; turn assertions that set the upper bits of a bit-vector to constants into a substitution "
             "that replaces the bit-vector with constant bits. Useful for minimizing circuits as many input bits to "
             "circuits are constant",
             "false", "smt");
    d.insert("bv.solver", CPK_UINT,
             "bit-vector solver engine: 0 - bit-blasting, 1 - polysat, 2 - intblast, requires sat.smt=true", "0",
             "smt");
    d.insert("arith.random_initial_value", CPK_BOOL,
             "use random initial values in the simplex-based procedure for linear arithmetic", "false", "smt");
    d.insert("arith.solver", CPK_UINT,
             "arithmetic solver: 0 - no solver, 1 - bellman-ford based solver (diff. logic only), 2 - simplex based "
             "solver, 3 - floyd-warshall based solver (diff. logic only) and no theory combination 4 - utvpi, 5 - "
             "infinitary lra, 6 - lra solver",
             "6", "smt");
    d.insert("arith.nl", CPK_BOOL,
             "(incomplete) nonlinear arithmetic support based on Groebner basis and interval propagation, relevant "
             "only if smt.arith.solver=2",
             "true", "smt");
    d.insert("arith.nl.nra", CPK_BOOL,
             "call nra_solver when incremental linearization does not produce a lemma, this option is ignored when "
    ...
    d.insert(
        "arith.nl.rounds", CPK_UINT,
        "threshold for number of (nested) final checks for non linear arithmetic, relevant only if smt.arith.solver=2",
        "1024", "smt");
    ...
    d.insert("arith.nl.horner_subs_fixed", CPK_UINT, "0 - no subs, 1 - substitute, 2 - substitute fixed zeros only",
             "2", "smt");
    d.insert("arith.nl.horner_frequency", CPK_UINT, "horner's call frequency", "4", "smt");
    d.insert("arith.nl.horner_row_length_limit", CPK_UINT,
             "row is disregarded by the heuristic if its length is longer than the value", "10", "smt");
    d.insert("arith.nl.grobner_row_length_limit", CPK_UINT,
             "row is disregarded by the heuristic if its length is longer than the value", "10", "smt");
    ...
    d.insert("arith.nl.grobner_max_simplified", CPK_UINT, "grobner's maximum number of simplifications", "10000",
             "smt");
    d.insert("arith.nl.grobner_cnfl_to_report", CPK_UINT, "grobner's maximum number of conflicts to report", "1",
             "smt");
    d.insert("arith.nl.gr_q", CPK_UINT, "grobner's quota", "10", "smt");
    d.insert("arith.nl.grobner_subs_fixed", CPK_UINT, "0 - no subs, 1 - substitute, 2 - substitute fixed zeros only",
             "1", "smt");
    d.insert("arith.nl.delay", CPK_UINT, "number of calls to final check before invoking bounded nlsat check", "10",
             "smt");
    ...
    d.insert("arith.propagation_mode", CPK_UINT,
             "0 - no propagation, 1 - propagate existing literals, 2 - refine finite bounds", "1", "smt");
    ...
    d.insert("arith.rep_freq", CPK_UINT, "the report frequency, in how many iterations print the cost and other info",
             "0", "smt");
    ...
    d.insert("arith.bprop_on_pivoted_rows", CPK_BOOL, "propagate bounds on rows changed by the pivot operation", "true",
             "smt");
    ...
    d.insert("up.persist_clauses", CPK_BOOL, "replay propagated clauses below the levels they are asserted", "true",
             "smt");
    ...
    d.insert("dack", CPK_UINT,
             "0 - disable dynamic ackermannization, 1 - expand Leibniz's axiom if a congruence is the root of a "
             "conflict, 2 - expand Leibniz's axiom if a congruence is used during conflict resolution",
             "1", "smt");
    ...
    d.insert("dack.gc", CPK_UINT, "Dynamic ackermannization garbage collection frequency (per conflict)", "2000",
             "smt");
    d.insert("dack.gc_inv_decay", CPK_DOUBLE, "Dynamic ackermannization garbage collection decay", "0.8", "smt");
    d.insert("dack.threshold", CPK_UINT,
             " number of times the congruence rule must be used before Leibniz's axiom is expanded", "10", "smt");
    d.insert("theory_case_split", CPK_BOOL,
             "Allow the context to use heuristics involving theory case splits, which are a set of literals of which "
             "exactly one can be assigned True. If this option is false, the context will generate extra axioms to "
             "enforce this instead.",
             "false", "smt");
    d.insert("string_solver", CPK_SYMBOL,
             "solver for string/sequence theories. options are: 'z3str3' (specialized string solver), 'seq' (sequence "
             "solver), 'auto' (use static features to choose best solver), 'empty' (a no-op solver that forces an "
             "answer unknown if strings were used), 'none' (no solver)",
             "seq", "smt");
    d.insert("core.validate", CPK_BOOL,
             "[internal] validate unsat core produced by SMT context. This option is intended for debugging", "false",
             "smt");
    ...
    d.insert("seq.max_unfolding", CPK_UINT,
             "maximal unfolding depth for checking string equations and regular expressions", "1000000000", "smt");
    d.insert("seq.min_unfolding", CPK_UINT,
             "initial bound for strings whose lengths are bounded by iterative deepening. Set this to a higher value "
             "if there are only models with larger string lengths",
             "1", "smt");
    d.insert("str.strong_arrangements", CPK_BOOL,
             "assert equivalences instead of implications when generating string arrangement axioms", "true", "smt");
    d.insert("str.aggressive_length_testing", CPK_BOOL,
             "prioritize testing concrete length values over generating more options", "false", "smt");
    d.insert("str.aggressive_value_testing", CPK_BOOL,
             "prioritize testing concrete string constant values over generating more options", "false", "smt");
    d.insert("str.aggressive_unroll_testing", CPK_BOOL,
             "prioritize testing concrete regex unroll counts over generating more options", "true", "smt");
    d.insert("str.fast_length_tester_cache", CPK_BOOL, "cache length tester constants instead of regenerating them",
             "false", "smt");
    d.insert("str.fast_value_tester_cache", CPK_BOOL, "cache value tester constants instead of regenerating them",
             "true", "smt");
    d.insert("str.string_constant_cache", CPK_BOOL,
             "cache all generated string constants generated from anywhere in theory_str", "true", "smt");
    d.insert(
        "theory_aware_branching", CPK_BOOL,
        "Allow the context to use extra information from theory solvers regarding literal branching prioritization.",
        "false", "smt");
    d.insert("str.overlap_priority", CPK_DOUBLE,
             "theory-aware priority for overlapping variable cases; use smt.theory_aware_branching=true", "-0.1",
             "smt");
    d.insert("str.regex_automata_difficulty_threshold", CPK_UINT, "difficulty threshold for regex automata heuristics",
             "1000", "smt");
    d.insert("str.regex_automata_intersection_difficulty_threshold", CPK_UINT,
             "difficulty threshold for regex intersection heuristics", "1000", "smt");
    d.insert("str.regex_automata_failed_automaton_threshold", CPK_UINT,
             "number of failed automaton construction attempts after which a full automaton is automatically built",
             "10", "smt");
    d.insert("str.regex_automata_failed_intersection_threshold", CPK_UINT,
             "number of failed automaton intersection attempts after which intersection is always computed", "10",
             "smt");
    d.insert("str.regex_automata_length_attempt_threshold", CPK_UINT,
             "number of length/path constraint attempts before checking unsatisfiability of regex terms", "10", "smt");
    d.insert("str.fixed_length_refinement", CPK_BOOL,
             "use abstraction refinement in fixed-length equation solver (Z3str3 only)", "false", "smt");
    d.insert("str.fixed_length_naive_cex", CPK_BOOL,
             "construct naive counterexamples when fixed-length model construction fails for a given length assignment "
             "(Z3str3 only)",
             "true", "smt");
    d.insert("core.minimize", CPK_BOOL, "minimize unsat core produced by SMT context", "false", "smt");
    d.insert("core.extend_patterns", CPK_BOOL,
             "extend unsat core with literals that trigger (potential) quantifier instances", "false", "smt");
    d.insert("core.extend_patterns.max_distance", CPK_UINT, "limits the distance of a pattern-extended unsat core",
             "4294967295", "smt");
    d.insert("core.extend_nonlocal_patterns", CPK_BOOL,
             "extend unsat cores with literals that have quantifiers with patterns that contain symbols which are not "
             "in the quantifier's body",
             "false", "smt");
    d.insert("lemma_gc_strategy", CPK_UINT,
             "lemma garbage collection strategy: 0 - fixed, 1 - geometric, 2 - at restart, 3 - none", "0", "smt");
    d.insert("dt_lazy_splits", CPK_UINT,
             "How lazy datatype splits are performed: 0- eager, 1- lazy for infinite types, 2- lazy", "1", "smt");
    d.insert("qsat_use_qel", CPK_BOOL, "Use QEL for lite quantifier elimination and model-based projection in QSAT",
             "true", "smt");
  }
  /*
  ...
  */
  ...
  unsigned arith_nl_grobner_expr_degree_growth() const {
    return p.get_uint("arith.nl.grobner_expr_degree_growth", g, 2u);
  }
  ...
  bool arith_nl_propagate_linear_monomials() const {
    return p.get_bool("arith.nl.propagate_linear_monomials", g, true);
  }
  ...
  bool up_persist_clauses() const { return p.get_bool("up.persist_clauses", g, true); }
  ...
  unsigned str_regex_automata_difficulty_threshold() const {
    return p.get_uint("str.regex_automata_difficulty_threshold", g, 1000u);
  }
  unsigned str_regex_automata_intersection_difficulty_threshold() const {
    return p.get_uint("str.regex_automata_intersection_difficulty_threshold", g, 1000u);
  }
  unsigned str_regex_automata_failed_automaton_threshold() const {
    return p.get_uint("str.regex_automata_failed_automaton_threshold", g, 10u);
  }
  unsigned str_regex_automata_failed_intersection_threshold() const {
    return p.get_uint("str.regex_automata_failed_intersection_threshold", g, 10u);
  }
  unsigned str_regex_automata_length_attempt_threshold() const {
    return p.get_uint("str.regex_automata_length_attempt_threshold", g, 10u);
  }
  ...
  unsigned core_extend_patterns_max_distance() const {
    return p.get_uint("core.extend_patterns.max_distance", g, 4294967295u);
  }
