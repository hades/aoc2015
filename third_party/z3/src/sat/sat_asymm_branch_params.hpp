...
struct sat_asymm_branch_params {
  params_ref const& p;
  params_ref g;
  sat_asymm_branch_params(params_ref const& _p = params_ref::get_empty()): p(_p), g(gparams::get_module("sat")) {}
  static void collect_param_descrs(param_descrs& d) {
    d.insert("asymm_branch", CPK_BOOL, "asymmetric branching", "true", "sat");
    d.insert("asymm_branch.rounds", CPK_UINT,
             "maximal number of rounds to run asymmetric branch simplifications if progress is made", "2", "sat");
    d.insert("asymm_branch.delay", CPK_UINT,
             "number of simplification rounds to wait until invoking asymmetric branch simplification", "1", "sat");
    d.insert("asymm_branch.sampled", CPK_BOOL,
             "use sampling based asymmetric branching based on binary implication graph", "true", "sat");
    d.insert("asymm_branch.limit", CPK_UINT, "approx. maximum number of literals visited during asymmetric branching",
             "100000000", "sat");
