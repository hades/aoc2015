...
struct sat_params {
  params_ref const& p;
  params_ref g;
  sat_params(params_ref const& _p = params_ref::get_empty()): p(_p), g(gparams::get_module("sat")) {}
  static void collect_param_descrs(param_descrs& d) {
    d.insert("max_memory", CPK_UINT, "maximum amount of memory in megabytes", "4294967295", "sat");
    d.insert("phase", CPK_SYMBOL,
             "phase selection strategy: always_false, always_true, basic_caching, random, caching, local_search",
             "caching", "sat");
    ...
    d.insert("branching.anti_exploration", CPK_BOOL, "apply anti-exploration heuristic for branch selection", "false",
             "sat");
    ...
    d.insert("gc.small_lbd", CPK_UINT, "learned clauses with small LBD are never deleted (only used in dyn_psm)", "3",
             "sat");
    d.insert("gc.k", CPK_UINT,
             "learned clauses that are inactive for k gc rounds are permanently deleted (only used in dyn_psm)", "7",
             "sat");
    ...
    d.insert("backtrack.conflicts", CPK_UINT, "number of conflicts before enabling chronological backtracking", "4000",
             "sat");
    ...
    d.insert("pb.solver", CPK_SYMBOL,
             "method for handling Pseudo-Boolean constraints: circuit (arithmetical circuit), sorting (sorting "
             "circuit), totalizer (use totalizer encoding), binary_merge, segmented, solver (use native solver)",
             "solver", "sat");
    d.insert("pb.min_arity", CPK_UINT, "minimal arity to compile pb/cardinality constraints to CNF", "9", "sat");
    d.insert("cardinality.encoding", CPK_SYMBOL,
             "encoding used for at-most-k constraints: grouped, bimander, ordered, unate, circuit", "grouped", "sat");
    d.insert("pb.resolve", CPK_SYMBOL, "resolution strategy for boolean algebra solver: cardinality, rounding",
             "cardinality", "sat");
    d.insert("pb.lemma_format", CPK_SYMBOL, "generate either cardinality or pb lemmas", "cardinality", "sat");
    d.insert("euf", CPK_BOOL, "enable euf solver (this feature is preliminary and not ready for general consumption)",
             "false", "sat");
    ...
    d.insert("ddfw.use_reward_pct", CPK_UINT, "percentage to pick highest reward variable when it has reward 0", "15",
             "sat");
    d.insert("ddfw.restart_base", CPK_UINT, "number of flips used a starting point for hesitant restart backoff",
             "100000", "sat");
    d.insert("ddfw.reinit_base", CPK_UINT,
             "increment basis for geometric backoff scheme of re-initialization of weights", "10000", "sat");
    ...
    d.insert("local_search_threads", CPK_UINT, "number of local search threads to find satisfiable solution", "0",
             "sat");
    ...
    d.insert("binspr", CPK_BOOL,
             "enable SPR inferences of binary propagation redundant clauses. This inprocessing step eliminates models",
             "false", "sat");
    ...
    d.insert("lookahead.cube.cutoff", CPK_SYMBOL,
             "cutoff type used to create lookahead cubes: depth, freevars, psat, adaptive_freevars, adaptive_psat",
             "depth", "sat");
    d.insert("lookahead.cube.fraction", CPK_DOUBLE,
             "adaptive fraction to create lookahead cubes. Used when lookahead.cube.cutoff is adaptive_freevars or "
             "adaptive_psat",
             "0.4", "sat");
    d.insert("lookahead.cube.depth", CPK_UINT,
             "cut-off depth to create cubes. Used when lookahead.cube.cutoff is depth.", "1", "sat");
    d.insert("lookahead.cube.freevars", CPK_DOUBLE,
             "cube free variable fraction. Used when lookahead.cube.cutoff is freevars", "0.8", "sat");
    ...
    d.insert("lookahead.cube.psat.trigger", CPK_DOUBLE,
             "trigger value to create lookahead cubes for PSAT cutoff. Used when lookahead.cube.cutoff is psat", "5",
             "sat");
    ...
    d.insert("lookahead_scores", CPK_BOOL,
             "extract lookahead scores. A utility that can only be used from the DIMACS front-end", "false", "sat");
    ...
    d.insert("lookahead_simplify.bca", CPK_BOOL, "add learned binary clauses as part of lookahead simplification",
             "true", "sat");
    d.insert("lookahead.global_autarky", CPK_BOOL,
             "prefer to branch on variables that occur in clauses that are reduced", "false", "sat");
    d.insert("lookahead.delta_fraction", CPK_DOUBLE,
             "number between 0 and 1, the smaller the more literals are selected for double lookahead", "1.0", "sat");
    d.insert("lookahead.reward", CPK_SYMBOL,
             "select lookahead heuristic: ternary, heule_schur (Heule Schur), heuleu (Heule Unit), unit, or march_cu",
             "march_cu", "sat");
  }
  /*
