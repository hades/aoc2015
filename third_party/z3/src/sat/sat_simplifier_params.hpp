...
struct sat_simplifier_params {
  params_ref const& p;
  params_ref g;
  sat_simplifier_params(params_ref const& _p = params_ref::get_empty()): p(_p), g(gparams::get_module("sat")) {}
  static void collect_param_descrs(param_descrs& d) {
    ...
    d.insert("blocked_clause_limit", CPK_UINT, "maximum number of literals visited during blocked clause elimination",
             "100000000", "sat");
    d.insert("override_incremental", CPK_BOOL,
             "override incremental safety gaps. Enable elimination of blocked clauses and variables even if solver is "
             "reused",
             "false", "sat");
    d.insert("resolution.limit", CPK_UINT, "approx. maximum number of literals visited during variable elimination",
             "500000000", "sat");
    d.insert("resolution.occ_cutoff", CPK_UINT,
             "first cutoff (on number of positive/negative occurrences) for Boolean variable elimination", "10", "sat");
    d.insert("resolution.occ_cutoff_range1", CPK_UINT,
             "second cutoff (number of positive/negative occurrences) for Boolean variable elimination, for problems "
             "containing less than res_cls_cutoff1 clauses",
             "8", "sat");
    d.insert("resolution.occ_cutoff_range2", CPK_UINT,
             "second cutoff (number of positive/negative occurrences) for Boolean variable elimination, for problems "
             "containing more than res_cls_cutoff1 and less than res_cls_cutoff2",
             "5", "sat");
    d.insert("resolution.occ_cutoff_range3", CPK_UINT,
             "second cutoff (number of positive/negative occurrences) for Boolean variable elimination, for problems "
             "containing more than res_cls_cutoff2",
             "3", "sat");
    d.insert("resolution.lit_cutoff_range1", CPK_UINT,
             "second cutoff (total number of literals) for Boolean variable elimination, for problems containing less "
             "than res_cls_cutoff1 clauses",
             "700", "sat");
    d.insert("resolution.lit_cutoff_range2", CPK_UINT,
             "second cutoff (total number of literals) for Boolean variable elimination, for problems containing more "
             "than res_cls_cutoff1 and less than res_cls_cutoff2",
             "400", "sat");
    d.insert("resolution.lit_cutoff_range3", CPK_UINT,
             "second cutoff (total number of literals) for Boolean variable elimination, for problems containing more "
             "than res_cls_cutoff2",
             "300", "sat");
    d.insert("resolution.cls_cutoff1", CPK_UINT,
             "limit1 - total number of problems clauses for the second cutoff of Boolean variable elimination",
             "100000000", "sat");
    d.insert("resolution.cls_cutoff2", CPK_UINT,
             "limit2 - total number of problems clauses for the second cutoff of Boolean variable elimination",
             "700000000", "sat");
    d.insert("elim_vars", CPK_BOOL, "enable variable elimination using resolution during simplification", "true",
             "sat");
    d.insert("elim_vars_bdd", CPK_BOOL, "enable variable elimination using BDD recompilation during simplification",
             "true", "sat");
    d.insert("elim_vars_bdd_delay", CPK_UINT,
             "delay elimination of variables using BDDs until after simplification round", "3", "sat");
    ...
    d.insert("probing_cache_limit", CPK_UINT, "cache binaries unless overall memory usage exceeds cache limit", "1024",
             "sat");
    ...
    d.insert("subsumption.limit", CPK_UINT,
             "approx. maximum number of literals visited during subsumption (and subsumption resolution)", "100000000",
             "sat");
  }
  /*
