...
struct opt_params {
  params_ref const& p;
  params_ref g;
  opt_params(params_ref const& _p = params_ref::get_empty()): p(_p), g(gparams::get_module("opt")) {}
  static void collect_param_descrs(param_descrs& d) {
    d.insert("optsmt_engine", CPK_SYMBOL, "select optimization engine: 'basic', 'symba'", "basic", "opt");
    d.insert("maxsat_engine", CPK_SYMBOL,
             "select engine for maxsat: 'core_maxsat', 'wmax', 'maxres', 'pd-maxres', 'maxres-bin', 'rc2'", "maxres",
             "opt");
    d.insert("priority", CPK_SYMBOL, "select how to prioritize objectives: 'lex' (lexicographic), 'pareto', 'box'",
             "lex", "opt");
    ...
    d.insert("enable_core_rotate", CPK_BOOL, "enable core rotation to both sample cores and correction sets", "false",
             "opt");
    ...
    d.insert("incremental", CPK_BOOL,
             "set incremental mode. It disables pre-processing and enables adding constraints in model event handler",
             "false", "opt");
    d.insert("pp.neat", CPK_BOOL,
             "use neat (as opposed to less readable, but faster) pretty printer when displaying context", "true",
             "opt");
    d.insert("pb.compile_equality", CPK_BOOL,
             "compile arithmetical equalities into pseudo-Boolean equality (instead of two inequalites)", "false",
             "opt");
    ...
    d.insert("maxres.max_core_size", CPK_UINT, "break batch of generated cores if size reaches this number", "3",
             "opt");
    d.insert("maxres.maximize_assignment", CPK_BOOL, "find an MSS/MCS to improve current assignment", "false", "opt");
    d.insert("maxres.max_correction_set_size", CPK_UINT,
             "allow generating correction set constraints up to maximal size", "3", "opt");
    d.insert("maxres.wmax", CPK_BOOL, "use weighted theory solver to constrain upper bounds", "false", "opt");
    d.insert("maxres.pivot_on_correction_set", CPK_BOOL,
             "reduce soft constraints if the current correction set is smaller than current core", "true", "opt");
  }
  /*
