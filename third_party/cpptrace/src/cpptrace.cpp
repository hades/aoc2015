...
raw_trace raw_trace::current(std::size_t skip) { return generate_raw_trace(skip + 1); }
...
void raw_trace::clear() { frames.clear(); }

bool raw_trace::empty() const noexcept { return frames.empty(); }

CPPTRACE_FORCE_NO_INLINE
object_trace object_trace::current(std::size_t skip) { return generate_object_trace(skip + 1); }
...
void object_trace::clear() { frames.clear(); }

bool object_trace::empty() const noexcept { return frames.empty(); }

object_frame stacktrace_frame::get_object_info() const { return detail::get_frame_object_info(raw_address); }
...
std::ostream& operator<<(std::ostream& stream, const stacktrace_frame& frame) { return stream << frame.to_string(); }

CPPTRACE_FORCE_NO_INLINE
stacktrace stacktrace::current(std::size_t skip) { return generate_trace(skip + 1); }

CPPTRACE_FORCE_NO_INLINE
stacktrace stacktrace::current(std::size_t skip, std::size_t max_depth) { return generate_trace(skip + 1, max_depth); }

void stacktrace::print() const { print(std::cerr, true); }

void stacktrace::print(std::ostream& stream) const { print(stream, true); }

void stacktrace::print(std::ostream& stream, bool color) const { print(stream, color, true, nullptr); }
...
void stacktrace::print(std::ostream& stream, bool color, bool newline_at_end, const char* header) const {
  if (color && ((&stream == &std::cout && isatty(stdout_fileno)) || (&stream == &std::cerr && isatty(stderr_fileno)))) {
...
void stacktrace::print_with_snippets() const { print_with_snippets(std::cerr, true); }

void stacktrace::print_with_snippets(std::ostream& stream) const { print_with_snippets(stream, true); }
...
void stacktrace::clear() { frames.clear(); }

bool stacktrace::empty() const noexcept { return frames.empty(); }
...
std::ostream& operator<<(std::ostream& stream, const stacktrace& trace) { return stream << trace.to_string(); }
...
CPPTRACE_FORCE_NO_INLINE
stacktrace generate_trace(std::size_t skip) { return generate_trace(skip + 1, SIZE_MAX); }
...
object_frame safe_object_frame::resolve() const { return detail::resolve_safe_object_frame(*this); }

void get_safe_object_frame(frame_ptr address, safe_object_frame* out) { detail::get_safe_object_frame(address, out); }

bool can_signal_safe_unwind() { return detail::has_safe_unwind(); }

std::string demangle(const std::string& name) { return detail::demangle(name); }
...
bool isatty(int fd) { return detail::isatty(fd); }
...
CPPTRACE_FORCE_NO_INLINE void print_terminate_trace() {
  generate_trace(1).print(std::cerr, isatty(stderr_fileno), true,
...
void register_terminate_handler() { std::set_terminate(terminate_handler); }
...
void absorb_trace_exceptions(bool absorb) { detail::absorb_trace_exceptions = absorb; }

void enable_inlined_call_resolution(bool enable) { detail::resolve_inlined_calls = enable; }

namespace experimental {
void set_cache_mode(cache_mode mode) { detail::cache_mode = mode; }
} // namespace experimental

namespace detail {
bool should_absorb_trace_exceptions() { return absorb_trace_exceptions; }

bool should_resolve_inlined_calls() { return resolve_inlined_calls; }

enum cache_mode get_cache_mode() { return cache_mode; }
...
raw_trace get_raw_trace_and_absorb(std::size_t skip, std::size_t max_depth) {
  try {
    ...
    if (!detail::should_absorb_trace_exceptions()) {
      ...
      std::fprintf(stderr, "Cpptrace: Exception occurred while resolving trace in cpptrace::exception object:\n%s\n",
                   e.what());
    }
    return raw_trace{};
  }
...
CPPTRACE_FORCE_NO_INLINE
raw_trace get_raw_trace_and_absorb(std::size_t skip) { return get_raw_trace_and_absorb(skip + 1, SIZE_MAX); }
...
lazy_trace_holder::~lazy_trace_holder() { clear(); }
// access
  ...
  if (!resolved) {
    ...
    *this = lazy_trace_holder(stacktrace{});
    try {
      ...
      if (!detail::should_absorb_trace_exceptions()) {
        // TODO: Append to message somehow?
        std::fprintf(stderr, "Exception occurred while resolving trace in cpptrace::exception object:\n%s\n", e.what());
      }
    }
  }
...
const stacktrace& lazy_trace_holder::get_resolved_trace() const {
  if (!resolved) {
    throw std::logic_error("cpptrace::detaillazy_trace_holder::get_resolved_trace called on unresolved const object");
  }
  return resolved_trace;
}
...
const char* lazy_exception::message() const noexcept { return "cpptrace::lazy_exception"; }

const stacktrace& lazy_exception::trace() const noexcept { return trace_holder.get_resolved_trace(); }

const char* exception_with_message::message() const noexcept { return user_message.c_str(); }

system_error::system_error(int error_code, std::string&& message_arg, raw_trace&& trace) noexcept
    : runtime_error(message_arg + ": " + std::error_code(error_code, std::generic_category()).message(),
                    std::move(trace)),
      ec(std::error_code(error_code, std::generic_category())) {}

const std::error_code& system_error::code() const noexcept { return ec; }
...
std::exception_ptr nested_exception::nested_ptr() const noexcept { return ptr; }
