...
struct die_object {
  ...
  die_object(Dwarf_Debug dbg, Dwarf_Die die): dbg(dbg), die(die) { ASSERT(dbg != nullptr); }
  ...
  operator bool() const { return die != nullptr; }
  ...
  void dwarf5_ranges(F callback) const {
    ...
    int res =
        wrap(dwarf_rnglists_get_rle_head, attr, form, offset, &head, &rnglists_entries, &dw_global_offset_of_rle_set);
    ...
    for (std::size_t i = 0; i < rnglists_entries; i++) {
      ...
      res = wrap(dwarf_get_rnglists_entry_fields_a, head, i, &entrylen, &rle_value_out, &raw1, &raw2, &unavailable,
                 &cooked1, &cooked2);
      if (res == DW_DLV_NO_ENTRY) {
        continue;
      }
  ...
  void dwarf4_ranges(Dwarf_Addr lowpc, F callback) const {
    ...
    VERIFY(wrap(dwarf_get_ranges_b, dbg, offset, die, nullptr, &ranges, &count, nullptr) == DW_DLV_OK);
    auto rangeswrapper =
        raii_wrap(ranges, [this, count](Dwarf_Ranges* ranges) { dwarf_dealloc_ranges(dbg, ranges, count); });
  ...
  void print() const {
    std::fprintf(stderr, "%08llx %s %s\n", to_ull(get_global_offset()), get_tag_name(), get_name().c_str());
  }
};
...
inline bool walk_die_list(const die_object& die, const std::function<bool(const die_object&)>& fn) {
...
// returns true if traversal should continue
inline bool walk_die_list_recursive(const die_object& die, const std::function<bool(const die_object&)>& fn) {
  return walk_die_list(die, [&fn](const die_object& die) {
...
public:
  static maybe_owned_die_object owned(die_object&& die) { return maybe_owned_die_object{std::move(die)}; }
  static maybe_owned_die_object ref(const die_object& die) { return maybe_owned_die_object{die}; }
