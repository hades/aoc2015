...
template <typename C> inline std::string join(const C& container, const std::string& delim) {
...
template <typename T, std::size_t N> struct byte_swapper;

template <typename T> struct byte_swapper<T, 1> {
  T operator()(T val) { return val; }
};

template <typename T> struct byte_swapper<T, 2> {
  T operator()(T val) { return (((val >> 8) & 0xff) | ((val & 0xff) << 8)); }
};

template <typename T> struct byte_swapper<T, 4> {
  T operator()(T val) {
...
template <typename T> struct byte_swapper<T, 8> {
  T operator()(T val) {
...
template <typename T, typename std::enable_if<std::is_integral<T>::value, int>::type = 0> T byteswap(T value) {
...
constexpr unsigned n_digits(unsigned value) noexcept { return value < 10 ? 1 : 1 + n_digits(value / 10); }
...
public:
  ...
  bool has_value() const { return holds_value; }

  explicit operator bool() const { return holds_value; }
...
template <typename T, typename E, typename std::enable_if<!std::is_same<T, E>::value, int>::type = 0> class Result {
  union {
    T value_;
    E error_;
  };
...
public:
  ...
  bool has_value() const { return active == member::value; }

  bool is_error() const { return active == member::error; }

  explicit operator bool() const { return has_value(); }

  NODISCARD optional<T> value() const& { return has_value() ? value_ : nullopt; }

  NODISCARD optional<E> error() const& { return is_error() ? error_ : nullopt; }

  NODISCARD optional<T> value() && { return has_value() ? std::move(value_) : nullopt; }

  NODISCARD optional<E> error() && { return is_error() ? std::move(error_) : nullopt; }
...
template <typename T> unsigned long long to_ull(T t) { return static_cast<unsigned long long>(t); }
template <typename T> frame_ptr to_frame_ptr(T t) { return static_cast<frame_ptr>(t); }

// A way to cast to U without "warning: useless cast to type"
template <typename U, typename V> U to(V v) { return static_cast<U>(v); }
...
// workaround a msvc bug https://developercommunity.visualstudio.com/t/MSVC-1938331290-preview-fails-to-comp/10505565
    ...
    ,
    typename std::enable_if<std::is_same<decltype(std::declval<D>()(std::declval<T>())), void>::value, int>::type = 0,
    typename std::enable_if<std::is_standard_layout<T>::value && std::is_trivial<T>::value, int>::type = 0,
...
template <typename T> class maybe_owned {
