...
std::vector<frame_ptr> capture_frames(std::size_t skip, std::size_t max_depth) {
  ...
  asm("label:\n\t"
      ...
      : [cEbp] "=r"(context.Ebp), [cEsp] "=r"(context.Esp), [cEip] "=r"(context.Eip));
#endif
#else
...
#error "Cpptrace: StackWalk64 not supported for this platform yet"
  ...
  while (trace.size() < max_depth) {
    if (!StackWalk64(machine_type, proc, thread, &frame, machine_type == IMAGE_FILE_MACHINE_I386 ? NULL : &context,
...
bool has_safe_unwind() { return false; }
} // namespace detail
} // namespace cpptrace
