...
private:
  ...
  template <typename... Args, typename... Args2,
            typename std::enable_if<std::is_same<decltype((void)std::declval<int(Args...)>()(
                                                     std::forward<Args2>(std::declval<Args2>())..., nullptr)),
                                                 void>::value,
                                    int>::type = 0>
...
public:
  ...
  dwarf_resolver(const std::string& object_path_, optional<skeleton_info> split_ = nullopt)
      : object_path(object_path_), skeleton(std::move(split_)) {
    ...
    auto ret = wrap(dwarf_init_path_a, object_path.c_str(), buffer.get(), CPPTRACE_MAX_PATH, DW_GROUPNUMBER_ANY,
                    universal_number, nullptr, nullptr, &dbg);
    if (ret == DW_DLV_OK) {
      ok = true;
...
private:
  ...
  void walk_compilation_units(const std::function<bool(const die_object&)>& fn) {
    ...
    while (true) {
      int ret = wrap(dwarf_next_cu_header_d, dbg, true, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr,
                     nullptr, &next_cu_header, &header_cu_type);
  ...
  void lazy_generate_cu_cache() {
    if (!generated_cu_cache) {
      ...
      std::sort(cu_cache.begin(), cu_cache.end(), [](const cu_entry& a, const cu_entry& b) { return a.low < b.low; });
  ...
  void get_inlines_info(const die_object& cu_die, const die_object& die, Dwarf_Addr pc, Dwarf_Half dwversion,
                    ...
                    [this, &cu_die, pc, dwversion, &inlines, &target_die, &current_obj_holder](const die_object& die) {
                      if (die.get_tag() == DW_TAG_inlined_subroutine && die.pc_in_die(dwversion, pc)) {
                        ...
                        if (file_i) {
                          ...
                          if (skeleton) {
                            line_table_opt = skeleton.unwrap().resolver.get_line_table(skeleton.unwrap().cu_die);
                          } else {
                        ...
                        inlines.push_back(stacktrace_frame{0,
  ...
  std::string retrieve_symbol_for_subprogram(const die_object& cu_die, const die_object& die, Dwarf_Addr pc,
                                             Dwarf_Half dwversion, std::vector<stacktrace_frame>& inlines) {
  ...
  CPPTRACE_FORCE_NO_INLINE_FOR_PROFILING
  bool retrieve_symbol_walk(const die_object& cu_die, const die_object& die, Dwarf_Addr pc, Dwarf_Half dwversion,
                            stacktrace_frame& frame, std::vector<stacktrace_frame>& inlines) {
    bool found = false;
    walk_die_list(die, [this, &cu_die, pc, dwversion, &frame, &inlines, &found](const die_object& die) {
      if (dump_dwarf) {
        std::fprintf(stderr, "-------------> %08llx %s %s\n", to_ull(die.get_global_offset()), die.get_tag_name(),
                     die.get_name().c_str());
      }
  ...
  CPPTRACE_FORCE_NO_INLINE_FOR_PROFILING
  void preprocess_subprograms(const die_object& die, Dwarf_Half dwversion, std::vector<subprogram_entry>& vec) {
    walk_die_list(die, [this, dwversion, &vec](const die_object& die) {
      switch (die.get_tag()) {
  ...
  CPPTRACE_FORCE_NO_INLINE_FOR_PROFILING
  void retrieve_symbol(const die_object& cu_die, Dwarf_Addr pc, Dwarf_Half dwversion, stacktrace_frame& frame,
                       std::vector<stacktrace_frame>& inlines) {
    if (get_cache_mode() == cache_mode::prioritize_memory) {
      ...
      if (it == subprograms_cache.end()) {
        ...
        std::sort(vec.begin(), vec.end(),
                  [](const subprogram_entry& a, const subprogram_entry& b) { return a.low < b.low; });
        subprograms_cache.emplace(off, std::move(vec));
        it = subprograms_cache.find(off);
      }
      auto& vec = it->second;
      auto vec_it = first_less_than_or_equal(
          vec.begin(), vec.end(), pc, [](Dwarf_Addr pc, const subprogram_entry& entry) { return pc < entry.low; });
  ...
  optional<std::reference_wrapper<line_table_info>> get_line_table(const die_object& cu_die) {
    ...
    if (it != line_tables.end()) {
      ...
      Dwarf_Line_Context line_context;
      int ret = wrap(dwarf_srclines_b, cu_die.get(), &version, &table_count, &line_context);
      ...
      if (get_cache_mode() == cache_mode::prioritize_speed) {
        ...
        VERIFY(wrap(dwarf_srclines_two_level_from_linecontext, line_context, &line_buffer, &line_count,
                    &linebuf_actuals, &linecount_actuals) == DW_DLV_OK);
        ...
        for (int i = 0; i < line_count; i++) {
          ...
          //     }
          // }
          line_entries.push_back({low_addr, line});
          i = j - 1;
        }
        // sort lines
        std::sort(line_entries.begin(), line_entries.end(),
                  [](const line_entry& a, const line_entry& b) { return a.low < b.low; });
      }
  ...
  CPPTRACE_FORCE_NO_INLINE_FOR_PROFILING
  void retrieve_line_info(const die_object& cu_die, Dwarf_Addr pc, stacktrace_frame& frame) {
    ...
    if (get_cache_mode() == cache_mode::prioritize_speed) {
      ...
      auto table_it = first_less_than_or_equal(line_entries.begin(), line_entries.end(), pc,
                                               [](Dwarf_Addr pc, const line_entry& entry) { return pc < entry.low; });
      // If the vector has been empty this can happen
      if (table_it != line_entries.end()) {
        ...
        if (!table_it->path) {
          char* filename = nullptr;
          VERIFY(wrap(dwarf_linesrc, line, &filename) == DW_DLV_OK);
          auto wrapper = raii_wrap(filename, [this](char* str) {
            if (str)
      ...
      Dwarf_Line* linebuf_actuals = nullptr;
      Dwarf_Signed linecount_actuals = 0;
      VERIFY(wrap(dwarf_srclines_two_level_from_linecontext, line_context, &line_buffer, &line_count, &linebuf_actuals,
                  &linecount_actuals) == DW_DLV_OK);
  ...
  optional<cu_info> lookup_cu(Dwarf_Addr pc) {
    ...
    if (get_cache_mode() == cache_mode::prioritize_memory) {
      ...
      walk_compilation_units([this, pc, &info](const die_object& cu_die) {
        ...
        // NOTE: If we have a corresponding skeleton, we assume we have one CU matching the skeleton CU
        if ((skeleton && skeleton.unwrap().cu_die.pc_in_die(skeleton.unwrap().dwversion, pc)) ||
            cu_die.pc_in_die(dwversion, pc)) {
          if (trace_dwarf) {
            std::fprintf(stderr, "pc in die %08llx %s (now searching for %08llx)\n", to_ull(cu_die.get_global_offset()),
                         cu_die.get_tag_name(), to_ull(pc));
          }
      ...
      // look up the cu
      auto vec_it = first_less_than_or_equal(cu_cache.begin(), cu_cache.end(), pc,
                                             [](Dwarf_Addr pc, const cu_entry& entry) { return pc < entry.low; });
  ...
  void perform_dwarf_fission_resolution(const die_object& cu_die, const optional<std::string>& dwo_name,
                                        const object_frame& object_frame_info, stacktrace_frame& frame,
                                        std::vector<stacktrace_frame>& inlines) {
    ...
    if (dwo_name) {
      ...
      if (get_cache_mode() == cache_mode::prioritize_memory) {
        dwarf_resolver resolver(path, skeleton_info{cu_die.clone(), dwversion, *this});
        res = resolver.resolve_frame(object_frame_info);
      } else {
        auto off = cu_die.get_global_offset();
        auto it = split_full_cu_resolvers.find(off);
        if (it == split_full_cu_resolvers.end()) {
          it = split_full_cu_resolvers
                   .emplace(off, std::unique_ptr<dwarf_resolver>(
                                     new dwarf_resolver(path, skeleton_info{cu_die.clone(), dwversion, *this})))
                   .first;
        }
  ...
  CPPTRACE_FORCE_NO_INLINE_FOR_PROFILING
  void resolve_frame_core(const object_frame& object_frame_info, stacktrace_frame& frame,
                          std::vector<stacktrace_frame>& inlines) {
...
public:
  ...
  frame_with_inlines resolve_frame(const object_frame& frame_info) override {
    if (!ok) {
      return {{frame_info.raw_address, frame_info.object_address, nullable<std::uint32_t>::null(),
               nullable<std::uint32_t>::null(), frame_info.object_path, "", false},
              {}};
    }
    ...
    if (trace_dwarf) {
      std::fprintf(stderr, "Starting resolution for %s %08llx\n", object_path.c_str(),
                   to_ull(frame_info.object_address));
    }
    std::vector<stacktrace_frame> inlines;
    resolve_frame_core(frame_info, frame, inlines);
