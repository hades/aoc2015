...
struct target_object {
  ...
  frame_with_inlines resolve_frame(const object_frame& frame_info, const std::string& symbol_name, std::size_t offset) {
    ...
    if (it != symbol_table.end()) {
      ...
      return {{frame_info.raw_address, frame_info.object_address, nullable<std::uint32_t>::null(),
               nullable<std::uint32_t>::null(), frame_info.object_path, symbol_name, false},
              {}};
    }
  }
};
...
public:
  debug_map_resolver(const std::string& source_object_path) {
    ...
    for (auto& entry: source_debug_map.unwrap_value()) {
      ...
      for (auto& symbol: map_entry_symbols) {
        symbols.push_back({symbol.source_address, symbol.size, std::move(symbol.name), nullable<uint64_t>::null(),
                           target_objects.size() - 1});
      }
    }
    // sort for binary lookup later
    std::sort(symbols.begin(), symbols.end(), [](const debug_map_symbol_info& a, const debug_map_symbol_info& b) {
  ...
  frame_with_inlines resolve_frame(const object_frame& frame_info) override {
    ...
    auto closest_symbol_it = first_less_than_or_equal(
        symbols.begin(), symbols.end(), frame_info.object_address,
        [](uint64_t pc, const debug_map_symbol_info& symbol) { return pc < symbol.source_address; });
      ...
      if (frame_info.object_address <= closest_symbol_it->source_address + closest_symbol_it->size) {
        return target_objects[closest_symbol_it->object_index].resolve_frame(
            {frame_info.raw_address,
             ...
             0, frame_info.object_path},
            closest_symbol_it->name, frame_info.object_address - closest_symbol_it->source_address);
      }
    }
