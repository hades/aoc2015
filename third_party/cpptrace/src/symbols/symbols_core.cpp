...
std::unordered_map<std::string, CollatedVec> collate_frames(const std::vector<object_frame>& frames,
  ...
  for (std::size_t i = 0; i < frames.size(); i++) {
    ...
    if (!entry.object_path.empty()) {
      entries[entry.object_path].emplace_back(entry, trace[i]);
    }
  }
  return entries;
}

std::unordered_map<std::string, collated_vec> collate_frames(const std::vector<object_frame>& frames,
                                                             std::vector<stacktrace_frame>& trace) {
  return collate_frames<collated_vec>(frames, trace);
}
std::unordered_map<std::string, collated_vec_with_inlines> collate_frames(const std::vector<object_frame>& frames,
...
void fill_blanks(std::vector<stacktrace_frame>& vec,
...
std::vector<stacktrace_frame> resolve_frames(const std::vector<object_frame>& frames) {
  ...
  return trace;
#else
#if defined(CPPTRACE_GET_SYMBOLS_WITH_LIBDL) || defined(CPPTRACE_GET_SYMBOLS_WITH_DBGHELP) ||                          \
...
#if defined(CPPTRACE_GET_SYMBOLS_WITH_LIBDWARF) || defined(CPPTRACE_GET_SYMBOLS_WITH_ADDR2LINE)
