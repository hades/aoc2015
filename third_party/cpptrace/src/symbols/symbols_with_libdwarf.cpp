...
std::vector<stacktrace_frame> flatten_inlines(std::vector<frame_with_inlines>& trace) {
  ...
  for (auto& entry: trace) {
    ...
    if (!entry.inlines.empty()) {
      ...
      final_trace.insert(final_trace.end(), std::make_move_iterator(entry.inlines.rbegin()),
...
std::vector<stacktrace_frame> resolve_frames(const std::vector<object_frame>& frames) {
  ...
  for (std::size_t i = 0; i < frames.size(); i++) {
    ...
    if (frame.frame == null_frame) {
      frame = {{dlframe.raw_address, dlframe.object_address, nullable<std::uint32_t>::null(),
                nullable<std::uint32_t>::null(), dlframe.object_path, "", false},
               {}};
    }
  }
  // flatten and finish
