...
#endif
#if !IS_APPLE
    execl(CPPTRACE_ADDR2LINE_PATH, CPPTRACE_ADDR2LINE_PATH, "-e", executable.c_str(), "-f", "-C", "-p", nullptr);
#else
    execl(CPPTRACE_ADDR2LINE_PATH, CPPTRACE_ADDR2LINE_PATH, "-o", executable.c_str(), "-fullPath", nullptr);
#endif
#endif
...
#error "CPPTRACE_ADDR2LINE_PATH must be defined if CPPTRACE_ADDR2LINE_SEARCH_SYSTEM_PATH is not"
#endif
  std::FILE* p = popen((CPPTRACE_ADDR2LINE_PATH " -e \"" + executable + "\" -fCp " + addresses).c_str(), "r");
#endif
  std::string output;
  constexpr int buffer_size = 4096;
  char buffer[buffer_size];
...
void update_trace(const std::string& line, std::size_t entry_index, const collated_vec& entries_vec) {
  ...
  VERIFY(object_end != std::string::npos, "Unexpected edge case while processing addr2line/atos output");
  ...
  VERIFY(filename_end != std::string::npos, "Unexpected edge case while processing addr2line/atos output");
  ...
  VERIFY(line_end == line.size() - 1, "Unexpected edge case while processing addr2line/atos output");
    ...
    for (const auto& entry: entries) {
      try {
        ...
        for (const auto& pair: entries_vec) {
          address_input += microfmt::format("{:h}{}", pair.first.get().object_address,
#if !IS_WINDOWS
                                            '\n'
#else
                                            ' '
