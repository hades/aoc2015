...
T get_info(ULONG type_index, HANDLE proc, ULONG64 modbase) {
  ...
  if (!SymGetTypeInfo(proc, modbase, type_index, static_cast<::IMAGEHLP_SYMBOL_TYPE_INFO>(SymType), &info)) {
    if (FAILABLE) {
      ...
      throw internal_error("SymGetTypeInfo failed: {}",
...
std::string get_info_wchar(ULONG type_index, HANDLE proc, ULONG64 modbase) {
  WCHAR* info;
  if (!SymGetTypeInfo(proc, modbase, type_index, static_cast<::IMAGEHLP_SYMBOL_TYPE_INFO>(SymType), &info)) {
    throw internal_error("SymGetTypeInfo failed: {}", std::system_error(GetLastError(), std::system_category()).what());
  }
...
static type_result lookup_type(ULONG type_index, HANDLE proc, ULONG64 modbase) {
  ...
  case SymTagEnum::SymTagArrayType: {
    DWORD underlying_type_id = get_info<DWORD, IMAGEHLP_SYMBOL_TYPE_INFO::TI_GET_TYPEID>(type_index, proc, modbase);
    DWORD length = get_info<DWORD, IMAGEHLP_SYMBOL_TYPE_INFO::TI_GET_COUNT>(type_index, proc, modbase);
    const auto type = lookup_type(underlying_type_id, proc, modbase);
    return {type.base, "[" + std::to_string(length) + "]" + type.extent};
  }
  case SymTagEnum::SymTagFunctionType: {
    DWORD return_type_id = get_info<DWORD, IMAGEHLP_SYMBOL_TYPE_INFO::TI_GET_TYPEID>(type_index, proc, modbase);
    DWORD n_children = get_info<DWORD, IMAGEHLP_SYMBOL_TYPE_INFO::TI_GET_COUNT, true>(type_index, proc, modbase);
    DWORD class_parent_id =
        get_info<DWORD, IMAGEHLP_SYMBOL_TYPE_INFO::TI_GET_CLASSPARENTID, true>(type_index, proc, modbase);
    ...
    if (n_children == 0) {
      ...
      std::size_t sz = sizeof(TI_FINDCHILDREN_PARAMS) + (n_children) * sizeof(TI_FINDCHILDREN_PARAMS::ChildId[0]);
  ...
  default:
    return {"<unknown type " + std::to_string(static_cast<std::underlying_type<SymTagEnum>::type>(tag)) + ">", ""};
  };
}
...
static BOOL __stdcall enumerator_callback(PSYMBOL_INFO symbol_info, ULONG, PVOID data) {
...
stacktrace_frame resolve_frame(HANDLE proc, frame_ptr addr) {
  ...
  if (SymFromAddr(proc, addr, &displacement.a, symbol)) {
    if (got_line) {
      ...
      DWORD n_children =
          get_info<DWORD, IMAGEHLP_SYMBOL_TYPE_INFO::TI_GET_COUNT, true>(symbol->TypeIndex, proc, symbol->ModBase);
    ...
    return {addr, object_frame.object_address, nullable<std::uint32_t>::null(), nullable<std::uint32_t>::null(), "", "",
            false};
  }
}
