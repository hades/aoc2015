...
#define CTRACE_FORMAT_PROLOGUE                                                                                         \
  _Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Wformat-security\"")
#define CTRACE_FORMAT_EPILOGUE _Pragma("clang diagnostic pop")
#elif defined(__GNUC_MINOR__)
#define CTRACE_FORMAT_PROLOGUE _Pragma("GCC diagnostic push") _Pragma("GCC diagnostic ignored \"-Wformat-security\"")
#define CTRACE_FORMAT_EPILOGUE _Pragma("GCC diagnostic pop")
#else
#define CTRACE_FORMAT_PROLOGUE
...
static bool is_empty(std::uint32_t pos) noexcept { return pos == invalid_pos; }

static bool is_empty(const char* str) noexcept { return !str || std::char_traits<char>::length(str) == 0; }
...
static void free_owning_string(const char* owned_string) noexcept {
  if (!owned_string)
    return; // Not necessary but eh
  delete[] owned_string;
}

static void free_owning_string(ctrace_owning_string& owned_string) noexcept { free_owning_string(owned_string.data); }
...
ctrace_object_trace ctrace_generate_object_trace(size_t skip, size_t max_depth) {
  try {
    std::vector<cpptrace::object_frame> trace =
        cpptrace::detail::get_frames_object_info(cpptrace::detail::capture_frames(skip + 1, max_depth));
...
ctrace_stacktrace ctrace_resolve_object_trace(const ctrace_object_trace* trace) {
  ...
  try {
    ...
    std::transform(trace->frames, trace->frames + trace->count, frames.begin(),
                   [](const ctrace_object_frame& frame) -> cpptrace::frame_ptr { return frame.raw_address; });
    std::vector<cpptrace::stacktrace_frame> resolved = cpptrace::detail::resolve_frames(frames);
    return ctrace::c_convert(resolved);
  } catch (...) { // NOSONAR
    // Don't check rethrow condition, it's risky.
...
ctrace_bool can_signal_safe_unwind() { return cpptrace::can_signal_safe_unwind(); }
...
void ctrace_print_stacktrace(const ctrace_stacktrace* trace, FILE* to, ctrace_bool use_color) {
  if (use_color && ((to == stdout && cpptrace::isatty(cpptrace::stdout_fileno)) ||
  ...
  for (std::size_t i = 0; i < trace->count; ++i) {
    ...
    if (frames[i].is_inline) {
      (void)std::fprintf(to, "%*s", int(ptr_len + 2), "(inlined)");
    } else {
      (void)std::fprintf(to, "%s0x%0*llx%s", blue, int(ptr_len), cpptrace::detail::to_ull(frames[i].raw_address),
                         reset);
    }
    if (!ctrace::is_empty(frames[i].symbol)) {
      (void)std::fprintf(to, " in %s%s%s", yellow, frames[i].symbol, reset);
    }
    if (!ctrace::is_empty(frames[i].filename)) {
      (void)std::fprintf(to, " at %s%s%s", green, frames[i].filename, reset);
      if (ctrace::is_empty(frames[i].line)) {
        ctrace::ffprintf(to, "\n");
        continue;
      }
      (void)std::fprintf(to, ":%s%llu%s", blue, cpptrace::detail::to_ull(frames[i].line), reset);
      if (ctrace::is_empty(frames[i].column)) {
        ctrace::ffprintf(to, "\n");
        continue;
      }
      (void)std::fprintf(to, ":%s%llu%s", blue, cpptrace::detail::to_ull(frames[i].column), reset);
    }
...
// utility::io
int ctrace_stdin_fileno(void) { return cpptrace::stdin_fileno; }

int ctrace_stderr_fileno(void) { return cpptrace::stderr_fileno; }

int ctrace_stdout_fileno(void) { return cpptrace::stdout_fileno; }

ctrace_bool ctrace_isatty(int fd) { return cpptrace::isatty(fd); }
...
void ctrace_enable_inlined_call_resolution(ctrace_bool enable) { cpptrace::enable_inlined_call_resolution(enable); }
