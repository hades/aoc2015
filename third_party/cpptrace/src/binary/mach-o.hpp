...
class mach_o {
  ...
  mach_o(file_wrapper file, const std::string& object_path, std::uint32_t magic)
      : file(std::move(file)), object_path(object_path), magic(magic) {}
...
public:
  ...
  void print_symbol_table_entry(const nlist_64& entry, const std::unique_ptr<char[]>& stringtab, std::size_t stringsize,
                                std::size_t j) const;
...
private:
  template <std::size_t Bits> Result<monostate, internal_error> load_mach();
  ...
  Result<std::unique_ptr<char[]>, internal_error> load_string_table(std::uint32_t offset,
