...
object_frame get_frame_object_info(frame_ptr address) {
  ...
  if (
    ...
    if (base.has_value()) {
      frame.object_address = address - reinterpret_cast<std::uintptr_t>(info.dli_fbase) + base.unwrap_value();
    } else {
      base.drop_error();
    }
  }
...
object_frame get_frame_object_info(frame_ptr address) {
  ...
  if (GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT | GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS,
                         reinterpret_cast<const char*>(address), &handle)) {
    frame.object_path = get_module_name(handle);
    auto base = get_module_image_base(frame.object_path);
    if (base.has_value()) {
      frame.object_address = address - reinterpret_cast<std::uintptr_t>(handle) + base.unwrap_value();
    } else {
      base.drop_error();
    }
  } else {
...
object_frame resolve_safe_object_frame(const safe_object_frame& frame) {
  ...
  return {frame.raw_address, frame.address_relative_to_object_start + base.unwrap_value(), frame.object_path};
}
} // namespace detail
} // namespace cpptrace
