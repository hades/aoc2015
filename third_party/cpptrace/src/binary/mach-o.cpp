...
bool is_fat_magic(std::uint32_t magic) { return magic == FAT_MAGIC || magic == FAT_CIGAM; }

// Based on https://github.com/AlexDenisov/segment_dumper/blob/master/main.c
// and https://lowlevelbits.org/parsing-mach-o-files/
bool is_magic_64(std::uint32_t magic) { return magic == MH_MAGIC_64 || magic == MH_CIGAM_64; }

bool should_swap_bytes(std::uint32_t magic) { return magic == MH_CIGAM || magic == MH_CIGAM_64 || magic == FAT_CIGAM; }

void swap_mach_header(mach_header_64& header) { swap_mach_header_64(&header, NX_UnknownByteOrder); }

void swap_mach_header(mach_header& header) { swap_mach_header(&header, NX_UnknownByteOrder); }

void swap_segment_command(segment_command_64& segment) { swap_segment_command_64(&segment, NX_UnknownByteOrder); }

void swap_segment_command(segment_command& segment) { swap_segment_command(&segment, NX_UnknownByteOrder); }

void swap_nlist(struct nlist& entry) { swap_nlist(&entry, 1, NX_UnknownByteOrder); }

void swap_nlist(struct nlist_64& entry) { swap_nlist_64(&entry, 1, NX_UnknownByteOrder); }
...
Result<std::uintptr_t, internal_error> mach_o::get_text_vmaddr() {
  for (const auto& command: load_commands) {
    if (command.cmd == LC_SEGMENT_64 || command.cmd == LC_SEGMENT) {
      auto segment = command.cmd == LC_SEGMENT_64 ? load_segment_command<64>(command.file_offset)
...
void mach_o::print_segments() const {
  ...
  for (const auto& command: load_commands) {
    if (command.cmd == LC_SEGMENT_64 || command.cmd == LC_SEGMENT) {
      auto segment_load = command.cmd == LC_SEGMENT_64 ? load_segment_command<64>(command.file_offset)
                                                       : load_segment_command<32>(command.file_offset);
...
void mach_o::print_symbol_table_entry(const nlist_64& entry, const std::unique_ptr<char[]>& stringtab,
                                      std::size_t stringsize, std::size_t j) const {
  const char* type = "";
  ...
  fprintf(stderr, "%5llu %8llx %2llx %7s %2llu %4llx %16llx %s\n", to_ull(j), to_ull(entry.n_un.n_strx),
          to_ull(entry.n_type), type, to_ull(entry.n_sect), to_ull(entry.n_desc), to_ull(entry.n_value),
          stringtab == nullptr             ? "Stringtab error"
          : entry.n_un.n_strx < stringsize ? stringtab.get() + entry.n_un.n_strx
  ...
  for (const auto& command: load_commands) {
    if (command.cmd == LC_SYMTAB) {
      ...
      for (std::size_t j = 0; j < symtab.nsyms; j++) {
        auto entry = bits == 32 ? load_symtab_entry<32>(symtab.symoff, j) : load_symtab_entry<64>(symtab.symoff, j);
        if (!entry) {
          fprintf(stderr, "error loading symtab entry\n");
          entry.drop_error();
          continue;
        }
        print_symbol_table_entry(entry.unwrap_value(), std::move(stringtab).value_or(std::unique_ptr<char[]>(nullptr)),
                                 symtab.strsize, j);
      }
    }
    i++;
...
Result<mach_o::debug_map, internal_error> mach_o::get_debug_map() {
  ...
  for (std::size_t j = 0; j < symtab.nsyms; j++) {
    auto load_entry = bits == 32 ? load_symtab_entry<32>(symtab.symoff, j) : load_symtab_entry<64>(symtab.symoff, j);
    if (!load_entry) {
...
Result<std::vector<mach_o::symbol_entry>, internal_error> mach_o::symbol_table() {
  ...
  for (std::size_t j = 0; j < symtab.nsyms; j++) {
    ...
    if ((entry.n_type & N_TYPE) == N_SECT) {
      ...
      symbols.push_back({entry.n_value, str.unwrap_value()});
    }
  }
  return symbols;
}
...
void mach_o::print_debug_map(const debug_map& debug_map) {
  for (const auto& entry: debug_map) {
    ...
    for (const auto& symbol: entry.second) {
      std::cerr << "    " << symbol.name << " " << std::hex << symbol.source_address << " " << symbol.size << std::dec
                << '\n';
    }
  }
}

template <std::size_t Bits> Result<monostate, internal_error> mach_o::load_mach() {
...
Result<monostate, internal_error> mach_o::load_fat_mach() {
  ...
  //     std::cerr<<"     "<<arch.cpusubtype<<" : "<<static_cast<cpu_subtype_t>(mhp->cpusubtype &
  //     ~CPU_SUBTYPE_MASK)<<std::endl; if(
  ...
  thread_local static struct LP(mach_header)* mhp = _NSGetMachExecuteHeader();
  fat_arch* best = NXFindBestFatArch(mhp->cputype, mhp->cpusubtype, fat_arches.data(), header.nfat_arch);
  if (best) {
...
Result<std::unique_ptr<char[]>, internal_error> mach_o::load_string_table(std::uint32_t offset,
...
bool mach_o::should_swap() const { return should_swap_bytes(magic); }
