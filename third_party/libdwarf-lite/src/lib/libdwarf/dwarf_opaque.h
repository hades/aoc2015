...
struct Dwarf_Debug_s {
  ...
  */
  ...
  void (*de_copy_word)(void* dw_targ, const void* dw_src, unsigned long dw_len);
...
int _dwarf_get_string_base_attr_value(Dwarf_Debug dbg, Dwarf_CU_Context context, Dwarf_Unsigned* sbase_out,
                                      Dwarf_Error* error);

int _dwarf_look_in_local_and_tied_by_index(Dwarf_Debug dbg, Dwarf_CU_Context context, Dwarf_Unsigned index,
...
int _dwarf_get_debugfission_for_offset(Dwarf_Debug dbg, Dwarf_Off offset_wanted, const char* keytype, /* "cu" or "tu" */
                                       Dwarf_Debug_Fission_Per_CU* percu_out, Dwarf_Error* error);

/* whichone: must be a valid DW_SECT* macro value. */
Dwarf_Unsigned _dwarf_get_dwp_extra_offset(struct Dwarf_Debug_Fission_Per_CU_s* dwp, unsigned whichone,
                                           Dwarf_Unsigned* size);
...
int _dwarf_get_addr_from_tied(Dwarf_Debug dbg, Dwarf_CU_Context context, Dwarf_Unsigned addrindex, Dwarf_Addr* addr_out,
                              Dwarf_Error* error);

int _dwarf_get_fission_addition_die(Dwarf_Die die, int dw_sect_index, Dwarf_Unsigned* offset, Dwarf_Unsigned* size,
                                    Dwarf_Error* error);
int _dwarf_get_addr_index_itself(int theform, Dwarf_Small* info_ptr, Dwarf_Debug dbg, Dwarf_CU_Context cu_context,
                                 Dwarf_Unsigned* val_out, Dwarf_Error* error);
Dwarf_Bool _dwarf_addr_form_is_indexed(int form);

int _dwarf_load_die_containing_section(Dwarf_Debug dbg, Dwarf_Bool is_info, Dwarf_Error* error);

int _dwarf_create_a_new_cu_context_record_on_list(Dwarf_Debug dbg, Dwarf_Debug_InfoTypes dis, Dwarf_Bool is_info,
                                                  Dwarf_Unsigned section_size, Dwarf_Unsigned new_cu_offset,
                                                  Dwarf_CU_Context* context_out, Dwarf_Die* cu_die_out,
                                                  Dwarf_Error* error);
Dwarf_Unsigned _dwarf_calculate_next_cu_context_offset(Dwarf_CU_Context cu_context);

int _dwarf_search_for_signature(Dwarf_Debug dbg, Dwarf_Sig8 sig, Dwarf_CU_Context* context_out, Dwarf_Error* error);

int _dwarf_merge_all_base_attrs_of_cu_die(Dwarf_Debug dbg, Dwarf_CU_Context context, Dwarf_Debug tieddbg,
                                          Dwarf_CU_Context* tiedcontext_out, Dwarf_Error* error);

void _dwarf_tied_destroy_free_node(void* node);
void _dwarf_destroy_group_map(Dwarf_Debug dbg);

int _dwarf_section_get_target_group(Dwarf_Debug dbg, unsigned obj_section_index, unsigned* groupnumber,
                                    Dwarf_Error* error);

int _dwarf_dwo_groupnumber_given_name(const char* name, unsigned* grpnum_out);

int _dwarf_section_get_target_group_from_map(Dwarf_Debug dbg, unsigned obj_section_index, unsigned* groupnumber_out,
                                             Dwarf_Error* error);

int _dwarf_insert_in_group_map(Dwarf_Debug dbg, unsigned groupnum, unsigned section_index, const char* name,
                               Dwarf_Error* error);

/*  returns TRUE/FALSE: meaning this section name is in
    map for this groupnum  or not.*/
int _dwarf_section_in_group_by_name(Dwarf_Debug dbg, const char* scn_name, unsigned groupnum);

int _dwarf_next_cu_header_internal(Dwarf_Debug dbg, Dwarf_Bool is_info, Dwarf_Die* cu_die_out,
                                   ...
                                   Dwarf_Half* extension_size, Dwarf_Sig8* signature, Dwarf_Bool* has_signature,
                                   Dwarf_Unsigned* typeoffset, Dwarf_Unsigned* next_cu_offset,
                                   Dwarf_Half* header_cu_type, Dwarf_Error* error);

/* Relates to .debug_addr */
int _dwarf_look_in_local_and_tied(Dwarf_Half attr_form, Dwarf_CU_Context context, Dwarf_Small* info_ptr,
                                  Dwarf_Addr* return_addr, Dwarf_Error* error);

int _dwarf_get_ranges_base_attr_from_tied(Dwarf_Debug dbg, Dwarf_CU_Context context, Dwarf_Unsigned* ranges_base_out,
                                          Dwarf_Unsigned* addr_base_out, Dwarf_Error* error);

int _dwarf_get_string_from_tied(Dwarf_Debug dbg, Dwarf_Unsigned offset, char** return_str, Dwarf_Error* error);

int _dwarf_valid_form_we_know(Dwarf_Unsigned at_form, Dwarf_Unsigned at_name);
int _dwarf_extract_local_debug_str_string_given_offset(Dwarf_Debug dbg, unsigned attrform, Dwarf_Unsigned offset,
                                                       char** return_str, Dwarf_Error* error);

int _dwarf_file_name_is_full_path(Dwarf_Small* fname);

/* This is libelf access to Elf object. */
extern int _dwarf_elf_setup(int fd, char* true_path_out_buffer, unsigned ftype, unsigned endian, unsigned offsetsize,
                            size_t filesize, unsigned groupnumber, Dwarf_Handler errhand, Dwarf_Ptr errarg,
                            Dwarf_Debug* dbg, Dwarf_Error* error);

/*  This is non-libelf Elf access */
extern int _dwarf_elf_nlsetup(int fd, char* true_path, unsigned ftype, unsigned endian, unsigned offsetsize,
                              size_t filesize, unsigned groupnumber, Dwarf_Handler errhand, Dwarf_Ptr errarg,
                              Dwarf_Debug* dbg, Dwarf_Error* error);
void _dwarf_destruct_elf_nlaccess(struct Dwarf_Obj_Access_Interface_a_s* aip);

extern int _dwarf_macho_setup(int fd, char* true_path, unsigned universalnumber, unsigned ftype, unsigned endian,
                              unsigned offsetsize, Dwarf_Unsigned filesize, unsigned groupnumber, Dwarf_Handler errhand,
                              Dwarf_Ptr errarg, Dwarf_Debug* dbg, Dwarf_Error* error);
void _dwarf_destruct_macho_access(struct Dwarf_Obj_Access_Interface_a_s* aip);

extern int _dwarf_pe_setup(int fd, char* path, unsigned ftype, unsigned endian, unsigned offsetsize, size_t filesize,
                           unsigned groupnumber, Dwarf_Handler errhand, Dwarf_Ptr errarg, Dwarf_Debug* dbg,
                           Dwarf_Error* error);
void _dwarf_destruct_pe_access(struct Dwarf_Obj_Access_Interface_a_s* aip);

void _dwarf_create_address_size_dwarf_error(Dwarf_Debug dbg, Dwarf_Error* error, Dwarf_Unsigned addrsize, int errcode,
                                            const char* errname);

extern Dwarf_Bool _dwarf_allow_formudata(unsigned form);
extern int _dwarf_formudata_internal(Dwarf_Debug dbg, Dwarf_Attribute attr, unsigned form, Dwarf_Byte_Ptr data,
                                     Dwarf_Byte_Ptr section_end, Dwarf_Unsigned* return_uval,
                                     Dwarf_Unsigned* bytes_read, Dwarf_Error* error);
...
int _dwarf_readr(int fd, char* buf, Dwarf_Unsigned size, Dwarf_Unsigned* sizeread);
int _dwarf_seekr(int fd, Dwarf_Unsigned loc, int seektype, Dwarf_Unsigned* out_loc);
int _dwarf_openr(const char* name);

int _dwarf_formblock_internal(Dwarf_Debug dbg, Dwarf_Attribute attr, Dwarf_CU_Context cu_context,
                              Dwarf_Block* return_block, Dwarf_Error* error);

int _dwarf_extract_data16(Dwarf_Debug dbg, Dwarf_Small* data, Dwarf_Small* section_start, Dwarf_Small* section_end,
                          Dwarf_Form_Data16* returned_val, Dwarf_Error* error);

int _dwarf_fill_in_attr_form_abtable(Dwarf_CU_Context context, Dwarf_Byte_Ptr abbrev_ptr, Dwarf_Byte_Ptr abbrev_end,
                                     Dwarf_Abbrev_List abbrev_list, Dwarf_Error* error);

int _dwarf_internal_find_die_given_sig8(Dwarf_Debug dbg, int context_level, Dwarf_Sig8* ref, Dwarf_Die* die_out,
                                        Dwarf_Bool* is_info, Dwarf_Error* error);
int _dwarf_internal_global_formref_b(Dwarf_Attribute attr, int context_level, Dwarf_Off* ret_offset,
                                     Dwarf_Bool* offset_is_info, Dwarf_Error* error);

int _dwarf_skip_leb128(char* /*leb*/, Dwarf_Unsigned* /*leblen*/, char* /*endptr*/);
