...
void _dwarf_create_area_len_error(Dwarf_Debug dbg, Dwarf_Error* error, Dwarf_Unsigned targ, Dwarf_Unsigned sectionlen);

#define SKIP_LEB128_CK(ptr, dbg, errptr, endptr)                                                                       \
  do {                                                                                                                 \
    Dwarf_Unsigned lu_leblen = 0;                                                                                      \
    int lu_res = 0;                                                                                                    \
    lu_res = _dwarf_skip_leb128((char*)(ptr), &lu_leblen, (char*)(endptr));                                            \
    if (lu_res == DW_DLV_ERROR) {                                                                                      \
      _dwarf_error_string((dbg), (errptr), DW_DLE_LEB_IMPROPER,                                                        \
...
#define DECODE_LEB128_UWORD_CK(ptr, value, dbg, errptr, endptr)                                                        \
  do {                                                                                                                 \
    ...
    int lu_res = 0;                                                                                                    \
    lu_res = dwarf_decode_leb128((char*)(ptr), &lu_leblen, &lu_local, (char*)(endptr));                                \
    if (lu_res == DW_DLV_ERROR) {                                                                                      \
      _dwarf_error_string((dbg), (errptr), DW_DLE_LEB_IMPROPER,                                                        \
...
#define DECODE_LEB128_SWORD_CK(ptr, value, dbg, errptr, endptr)                                                        \
  do {                                                                                                                 \
    ...
    int lu_res = 0;                                                                                                    \
    lu_res = dwarf_decode_signed_leb128((char*)(ptr), &uleblen, &local, (char*)(endptr));                              \
    if (lu_res == DW_DLV_ERROR) {                                                                                      \
      _dwarf_error_string((dbg), (errptr), DW_DLE_LEB_IMPROPER,                                                        \
...
#define DECODE_LEB128_SWORD_LEN_CK(ptr, value, leblen, dbg, errptr, endptr)                                            \
  do {                                                                                                                 \
    ...
    int lu_res = 0;                                                                                                    \
    lu_res = dwarf_decode_signed_leb128((char*)(ptr), &lu_leblen, &lu_local, (char*)(endptr));                         \
    if (lu_res == DW_DLV_ERROR) {                                                                                      \
      _dwarf_error_string((dbg), (errptr), DW_DLE_LEB_IMPROPER,                                                        \
...
#define IS_INVALID_DBG(d) ((!(d) || ((d)->de_magic != DBG_IS_VALID)) ? TRUE : FALSE)
  ...
  do {                                                                                                                 \
    if (!(cd_dbg) || (cd_dbg)->de_magic != DBG_IS_VALID) {                                                             \
      _dwarf_error_string(NULL, (cd_er), DW_DLE_DBG_NULL,                                                              \
                          "DW_DLE_DBG_NULL: "                                                                          \
                          "dbg argument to " cd_funcname "either null or it contains"                                  \
                          "a stale Dwarf_Debug pointer");                                                              \
      return DW_DLV_ERROR;                                                                                             \
    }                                                                                                                  \
  } while (0)
...
#define READ_UNALIGNED_CK(dbg, dest, desttype, source, length, error, endptr)                                          \
  do {                                                                                                                 \
    ...
    if (readend < (source)) {                                                                                          \
      _dwarf_error_string((dbg), (error), DW_DLE_READ_BIGENDIAN_ERROR,                                                 \
    ...
    (dbg)->de_copy_word((((char*)(&_ltmp)) + sizeof(_ltmp) - (length)), (source), (unsigned long)(length));            \
    (dest) = _ltmp;                                                                                                    \
  } while (0)
#else /* LITTLE ENDIAN */
#define READ_UNALIGNED_CK(dbg, dest, desttype, source, length, error, endptr)                                          \
  do {                                                                                                                 \
    ...
    Dwarf_Byte_Ptr readend = (source) + (length);                                                                      \
    if (readend < (source)) {                                                                                          \
      _dwarf_error_string((dbg), (error), DW_DLE_READ_LITTLEENDIAN_ERROR,                                              \
    ...
    (dbg)->de_copy_word((char*)(&_ltmp), (source), (unsigned long)(length));                                           \
    (dest) = _ltmp;                                                                                                    \
  } while (0)
#endif
...
#define READ_AREA_LENGTH_CK(r_dbg, w_target, r_targtype, rw_src_data_p, w_length_size, w_exten_size, w_error,          \
                            r_sectionlen, r_endptr)                                                                    \
  do {                                                                                                                 \
    READ_UNALIGNED_CK(r_dbg, w_target, r_targtype, rw_src_data_p, ORIGINAL_DWARF_OFFSET_SIZE, w_error, r_endptr);      \
    if ((w_target) == DISTINGUISHED_VALUE) {                                                                           \
      ...
      READ_UNALIGNED_CK(r_dbg, w_target, r_targtype, rw_src_data_p, DISTINGUISHED_VALUE_OFFSET_SIZE, w_error,          \
                        r_endptr);                                                                                     \
      if ((w_target) > (r_sectionlen)) {                                                                               \
        _dwarf_create_area_len_error((r_dbg), (w_error), (w_target), (r_sectionlen));                                  \
        return DW_DLV_ERROR;                                                                                           \
      }                                                                                                                \
...
int _dwarf_format_TAG_err_msg(Dwarf_Debug dbg, Dwarf_Unsigned tag, const char* m, Dwarf_Error* error);

int _dwarf_get_size_of_val(Dwarf_Debug dbg, Dwarf_Unsigned form, Dwarf_Half cu_version, Dwarf_Half address_size,
                           Dwarf_Small* val_ptr, int v_length_size, Dwarf_Unsigned* size_out,
                           Dwarf_Small* section_end_ptr, Dwarf_Error* error);
...
int _dwarf_get_abbrev_for_code(struct Dwarf_CU_Context_s* abcom, Dwarf_Unsigned code, Dwarf_Abbrev_List* list_out,
                               Dwarf_Unsigned* highest_known_code, Dwarf_Error* error);
...
*/
int _dwarf_check_string_valid(Dwarf_Debug dbg, void* areaptr, void* startptr, void* endptr, int suggested_error,
                              Dwarf_Error* error);

int _dwarf_length_of_cu_header(Dwarf_Debug dbg, Dwarf_Unsigned offset, Dwarf_Bool is_info,
                               Dwarf_Unsigned* area_length_out, Dwarf_Error* error);
...
void _dwarf_free_abbrev_hash_table_contents(struct Dwarf_Hash_Table_s* hash_table, Dwarf_Bool keep_abbrev_content);
...
void _dwarf_error_mv_s_to_t(Dwarf_Debug dbgs, Dwarf_Error* errs, Dwarf_Debug dbgt, Dwarf_Error* errt);

int _dwarf_internal_get_die_comp_dir(Dwarf_Die die, const char** compdir_out, const char** comp_name_out,
                                     Dwarf_Error* error);

int _dwarf_what_section_are_we(Dwarf_Debug dbg, Dwarf_Small* our_pointer, const char** section_name_out,
                               Dwarf_Small** sec_start_ptr_out, Dwarf_Unsigned* sec_len_out,
                               Dwarf_Small** sec_end_ptr_out);

/*  wrappers return either DW_DLV_OK or DW_DLV_ERROR.
    Never DW_DLV_NO_ENTRY. */
int _dwarf_read_unaligned_ck_wrapper(Dwarf_Debug dbg, Dwarf_Unsigned* out_value, Dwarf_Small* readfrom, int readlength,
                                     Dwarf_Small* end_arange, Dwarf_Error* err);
int _dwarf_read_area_length_ck_wrapper(Dwarf_Debug dbg, Dwarf_Unsigned* out_value, Dwarf_Small** readfrom,
                                       int* length_size_out, int* exten_size_out, Dwarf_Unsigned sectionlength,
                                       Dwarf_Small* endsection, Dwarf_Error* err);
int _dwarf_leb128_uword_wrapper(Dwarf_Debug dbg, Dwarf_Small** startptr, Dwarf_Small* endptr, Dwarf_Unsigned* out_value,
                                Dwarf_Error* error);
int _dwarf_leb128_sword_wrapper(Dwarf_Debug dbg, Dwarf_Small** startptr, Dwarf_Small* endptr, Dwarf_Signed* out_value,
                                Dwarf_Error* error);
