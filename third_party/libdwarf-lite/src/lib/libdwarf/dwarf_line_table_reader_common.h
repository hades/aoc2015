...
static unsigned char dwarf_standard_opcode_operand_count[STANDARD_OPERAND_COUNT_TWO_LEVEL] = {
    /* DWARF2 */
    0,
    1,
    1,
...
static unsigned char dwarf_arm_standard_opcode_operand_count[STANDARD_OPERAND_COUNT_DWARF3] = {
    /* DWARF2 */
    0, 1, 1, 1, 1, 0, 0, 0, 0, /* <<< --- this is wrong */
    /* Following are new for DWARF3. */
    0, 0, 1};
...
static int operandmismatch(unsigned char* table, unsigned table_length, unsigned char* linetable, unsigned check_count,
...
/*  Encapsulates DECODE_LEB128_UWORD_CK
    ...
    in case of problems. */
static int read_uword_de(Dwarf_Small** lp, Dwarf_Unsigned* out_p, Dwarf_Debug dbg, Dwarf_Error* err,
                         Dwarf_Small* lpend) {
  Dwarf_Small* inptr = *lp;
  Dwarf_Unsigned out = 0;
  DECODE_LEB128_UWORD_CK(inptr, out, dbg, err, lpend);
  *lp = inptr;
  *out_p = out;
  return DW_DLV_OK;
}
...
static void IssueExpError(Dwarf_Debug dbg, Dwarf_Error* err, const char* msg, Dwarf_Unsigned val) {
  dwarfstring m;
  char buf[200];
  ...
  _dwarf_error_string(dbg, err, DW_DLE_LINE_TABLE_BAD, dwarfstring_string(&m));
  dwarfstring_destructor(&m);
}

static int read_sword_de(Dwarf_Small** lp, Dwarf_Signed* out_p, Dwarf_Debug dbg, Dwarf_Error* err, Dwarf_Small* lpend) {
  Dwarf_Small* inptr = *lp;
  Dwarf_Signed out = 0;
  DECODE_LEB128_SWORD_CK(inptr, out, dbg, err, lpend);
  *lp = inptr;
  *out_p = out;
  return DW_DLV_OK;
}
...
static int _dwarf_read_line_table_header(Dwarf_Debug dbg, Dwarf_CU_Context cu_context, Dwarf_Small* section_start,
                                         Dwarf_Small* data_start, Dwarf_Unsigned section_length,
                                         Dwarf_Small** updated_data_start_out, Dwarf_Line_Context line_context,
                                         Dwarf_Small** bogus_bytes_ptr, Dwarf_Unsigned* bogus_bytes, Dwarf_Error* err,
                                         int* err_count_out) {
  ...
  READ_AREA_LENGTH_CK(dbg, total_length, Dwarf_Unsigned, line_ptr, local_length_size, local_extension_size, err,
                      section_length, section_end);
  line_ptr_end = line_ptr + total_length;
  line_context->lc_line_ptr_end = line_ptr_end;
  line_context->lc_length_field_length = local_length_size + local_extension_size;
  ...
  if (line_ptr_end > section_end) {
    ...
    dwarfstring_append_printf_u(&m,
                                ...
                                " %" DW_PR_DUu " bytes",
                                total_length);
    _dwarf_error_string(dbg, err, DW_DLE_DEBUG_LINE_LENGTH_BAD, dwarfstring_string(&m));
    dwarfstring_destructor(&m);
    return DW_DLV_ERROR;
  }
  ...
  /* READ_UNALIGNED_CK(dbg, version, Dwarf_Half,
      ...
      err,line_ptr_end); */
  res = _dwarf_read_unaligned_ck_wrapper(dbg, &htmp, line_ptr, DWARF_HALF_SIZE, line_ptr_end, err);
  if (res == DW_DLV_ERROR) {
    return res;
  }
  ...
  line_ptr += DWARF_HALF_SIZE;
  if (version != DW_LINE_VERSION2 && version != DW_LINE_VERSION3 && version != DW_LINE_VERSION4 &&
      version != DW_LINE_VERSION5 && version != EXPERIMENTAL_LINE_TABLES_VERSION) {
  ...
  if (version == DW_LINE_VERSION5) {
    ...
    line_context->lc_segment_selector_size = *(unsigned char*)line_ptr;
    line_ptr = line_ptr + sizeof(Dwarf_Small);
  } else {
    line_context->lc_address_size = cu_context->cc_address_size;
    line_context->lc_segment_selector_size = cu_context->cc_segment_selector_size;
  }

  READ_UNALIGNED_CK(dbg, prologue_length, Dwarf_Unsigned, line_ptr, local_length_size, err, line_ptr_end);
  ...
  line_context->lc_minimum_instruction_length = *(unsigned char*)line_ptr;
  line_ptr = line_ptr + sizeof(Dwarf_Small);

  if (version == DW_LINE_VERSION4 || version == DW_LINE_VERSION5 || version == EXPERIMENTAL_LINE_TABLES_VERSION) {
    if (line_ptr >= line_ptr_end) {
      _dwarf_error(dbg, err, DW_DLE_DEBUG_LINE_LENGTH_BAD);
      return DW_DLV_ERROR;
    }
    line_context->lc_maximum_ops_per_instruction = *(unsigned char*)line_ptr;
    line_ptr = line_ptr + sizeof(Dwarf_Small);
  }
  if (line_ptr >= line_ptr_end) {
    _dwarf_error(dbg, err, DW_DLE_DEBUG_LINE_LENGTH_BAD);
  ...
  }
  {
    ...
    if (line_context->lc_std_op_count > tab_count) {
      _dwarf_print_header_issue(dbg, "Too many standard operands in linetable header: ", data_start,
                                line_context->lc_std_op_count, 0, 0, 0, err_count_out);
      check_count = tab_count;
    }
    if ((line_context->lc_opcode_length_table + check_count) >= section_end) {
    ...
    }
    {
      ...
      int mismatch = operandmismatch(dwarf_standard_opcode_operand_count, tab_count,
                                     line_context->lc_opcode_length_table, check_count, &entrynum, &tabv, &linetabv);
      if (mismatch) {
        if (err_count_out) {
          _dwarf_print_header_issue(dbg, "standard-operands did not match, checked", data_start, check_count, entrynum,
                                    tabv, linetabv, err_count_out);
        }
        if (check_count > sizeof(dwarf_arm_standard_opcode_operand_count)) {
          check_count = sizeof(dwarf_arm_standard_opcode_operand_count);
        }
        mismatch =
            operandmismatch(dwarf_arm_standard_opcode_operand_count, sizeof(dwarf_arm_standard_opcode_operand_count),
                            line_context->lc_opcode_length_table, check_count, &entrynum, &tabv, &linetabv);
        if (!mismatch && err_count_out) {
          _dwarf_print_header_issue(dbg, "arm (incorrect) operands in use: ", data_start, check_count, entrynum, tabv,
                                    linetabv, err_count_out);
        }
      }
      if (!mismatch) {
        if (version == 2) {
          if (line_context->lc_std_op_count == STANDARD_OPERAND_COUNT_DWARF3) {
            _dwarf_print_header_issue(dbg,
                                      "standard DWARF3 operands matched,"
                                      " but is DWARF2 linetable: count",
                                      data_start, check_count, 0, 0, 0, err_count_out);
          }
        }
  ...
  if (version < DW_LINE_VERSION5) {
    ...
    line_context->lc_include_directories = malloc(sizeof(Dwarf_Small*) * directories_malloc);
    if (line_context->lc_include_directories == NULL) {
      _dwarf_error(dbg, err, DW_DLE_ALLOC_FAIL);
      return DW_DLV_ERROR;
    }
    memset(line_context->lc_include_directories, 0, sizeof(Dwarf_Small*) * directories_malloc);
    ...
    while ((*(char*)line_ptr) != '\0') {
      if (directories_count >= directories_malloc) {
        ...
        Dwarf_Small** newdirs = realloc(line_context->lc_include_directories, bytesalloc);
        ...
        memset(newdirs + directories_malloc, 0, sizeof(Dwarf_Small*) * directories_malloc);
        directories_malloc = expand;
        line_context->lc_include_directories = newdirs;
      }
      line_context->lc_include_directories[directories_count] = line_ptr;
      res = _dwarf_check_string_valid(dbg, data_start, line_ptr, line_ptr_end, DW_DLE_LINE_STRING_BAD, err);
      if (res != DW_DLV_OK) {
        return res;
      }
  ...
  if (version < DW_LINE_VERSION5) {
    ...
    while (*(char*)line_ptr != '\0') {
      ...
      currfile = (Dwarf_File_Entry)malloc(sizeof(struct Dwarf_File_Entry_s));
      if (currfile == NULL) {
        _dwarf_error(dbg, err, DW_DLE_ALLOC_FAIL);
        return DW_DLV_ERROR;
      }
      ...
      resl = _dwarf_check_string_valid(dbg, data_start, line_ptr, line_ptr_end, DW_DLE_LINE_STRING_BAD, err);
      if (resl != DW_DLV_OK) {
        return resl;
      }
      line_ptr = line_ptr + strlen((char*)line_ptr) + 1;
      /*  DECODE_LEB128_UWORD_CK(line_ptr, utmp,dbg,
          err,line_ptr_end); */
      res = read_uword_de(&line_ptr, &utmp, dbg, err, line_ptr_end);
      if (res == DW_DLV_ERROR) {
        return DW_DLV_ERROR;
      }
      ...
      res = read_uword_de(&line_ptr, &lastmod, dbg, err, line_ptr_end);
      if (res == DW_DLV_ERROR) {
        return DW_DLV_ERROR;
      }
      ...
      DECODE_LEB128_UWORD_CK(line_ptr, file_length, dbg, err, line_ptr_end);
  ...
  if (version == EXPERIMENTAL_LINE_TABLES_VERSION) {
    ...
    READ_UNALIGNED_CK(dbg, logicals_table_offset, Dwarf_Unsigned, line_ptr, local_length_size, err, line_ptr_end);
    line_context->lc_logicals_table_offset = logicals_table_offset;
    line_ptr += local_length_size;
    READ_UNALIGNED_CK(dbg, actuals_table_offset, Dwarf_Unsigned, line_ptr, local_length_size, err, line_ptr_end);
  ...
  if (version == DW_LINE_VERSION5 || version == EXPERIMENTAL_LINE_TABLES_VERSION) {
    ...
    line_context->lc_directory_entry_format_count = directory_format_count;
    line_ptr = line_ptr + sizeof(Dwarf_Small);
    if (directory_format_count > 0) {
      format_values = malloc(sizeof(struct Dwarf_Unsigned_Pair_s) * directory_format_count);
      if (format_values == NULL) {
        _dwarf_error(dbg, err, DW_DLE_ALLOC_FAIL);
        return DW_DLV_ERROR;
      }
      for (i = 0; i < directory_format_count; i++) {
        dres = read_uword_de(&line_ptr, &format_values[i].up_first, dbg, err, line_ptr_end);
        if (dres != DW_DLV_OK) {
          free(format_values);
          format_values = 0;
          return dres;
        }
        dres = read_uword_de(&line_ptr, &format_values[i].up_second, dbg, err, line_ptr_end);
        if (dres != DW_DLV_OK) {
          free(format_values);
          format_values = 0;
          return dres;
    ...
    dres = read_uword_de(&line_ptr, &directories_count, dbg, err, line_ptr_end);
    if (dres != DW_DLV_OK) {
      free(format_values);
      format_values = 0;
      return dres;
    ...
    line_context->lc_include_directories = malloc(sizeof(Dwarf_Small*) * directories_count);
    if (line_context->lc_include_directories == NULL) {
      free(format_values);
      format_values = 0;
      _dwarf_error(dbg, err, DW_DLE_ALLOC_FAIL);
    ...
    if (directory_format_count == 0 && directories_count > 0) {
      free(format_values);
      format_values = 0;
      _dwarf_error_string(dbg, err, DW_DLE_DIRECTORY_FORMAT_COUNT_VS_DIRECTORIES_MISMATCH,
                          "DW_DLE_DIRECTORY_FORMAT_COUNT_"
                          "VS_DIRECTORIES_MISMATCH"
                          ": format count is zero yet directories count > 0");
      return DW_DLV_ERROR;
    }
    memset(line_context->lc_include_directories, 0, sizeof(Dwarf_Small*) * directories_count);

    for (i = 0; i < directories_count; i++) {
      for (j = 0; j < directory_format_count; j++) {
        Dwarf_Unsigned lntype = format_values[j].up_first;
        Dwarf_Unsigned lnform = format_values[j].up_second;
        if (line_ptr >= line_ptr_end) {
          free(format_values);
          format_values = 0;
        ...
        case DW_LNCT_path: {
          char* inc_dir_ptr = 0;
          res = _dwarf_decode_line_string_form(dbg, lntype, lnform, local_length_size, &line_ptr, line_ptr_end,
                                               &inc_dir_ptr, err);
          if (res != DW_DLV_OK) {
            free(format_values);
            format_values = 0;
          ...
          line_context->lc_include_directories[i] = (unsigned char*)inc_dir_ptr;
          break;
        }
        default:
          free(format_values);
  ...
  if (version == DW_LINE_VERSION5 || version == EXPERIMENTAL_LINE_TABLES_VERSION) {
    ...
    line_context->lc_file_name_format_count = filename_format_count;
    line_ptr = line_ptr + sizeof(Dwarf_Small);
    filename_entry_pairs = malloc(sizeof(struct Dwarf_Unsigned_Pair_s) * filename_format_count);
    ...
    for (i = 0; i < filename_format_count; i++) {
      dres = read_uword_de(&line_ptr, &filename_entry_pairs[i].up_first, dbg, err, line_ptr_end);
      if (dres != DW_DLV_OK) {
        free(filename_entry_pairs);
        return dres;
      }
      dres = read_uword_de(&line_ptr, &filename_entry_pairs[i].up_second, dbg, err, line_ptr_end);
      if (dres != DW_DLV_OK) {
        free(filename_entry_pairs);
        return dres;
      }
    ...
    /* DECODE_LEB128_UWORD_CK(line_ptr, files_count,
        dbg,err,line_ptr_end); */
    dres = read_uword_de(&line_ptr, &files_count, dbg, err, line_ptr_end);
    if (dres != DW_DLV_OK) {
      free(filename_entry_pairs);
      return dres;
    }
    ...
    for (i = 0; i < files_count; i++) {
      ...
      curline = (Dwarf_File_Entry)malloc(sizeof(struct Dwarf_File_Entry_s));
      if (curline == NULL) {
        free(filename_entry_pairs);
        _dwarf_error_string(dbg, err, DW_DLE_ALLOC_FAIL,
                            "DW_DLE_ALLOC_FAIL: "
      ...
      for (j = 0; j < filename_format_count; j++) {
        ...
        Dwarf_Unsigned lntype = filename_entry_pairs[j].up_first;
        Dwarf_Unsigned lnform = filename_entry_pairs[j].up_second;
        ...
        */
        ...
        case DW_LNCT_GNU_decl_file: /* FORM udata*/
          res = _dwarf_decode_line_udata_form(dbg, lntype, lnform, &line_ptr, &dirindex, line_ptr_end, err);
          if (res != DW_DLV_OK) {
            free(filename_entry_pairs);
            return res;
          }
        ...
        case DW_LNCT_GNU_subprogram_name: {
          res = _dwarf_decode_line_string_form(dbg, lntype, lnform, local_length_size, &line_ptr, line_ptr_end,
                                               (char**)&curline->fi_gnu_subprogram_name, err);
          if (res != DW_DLV_OK) {
            free(filename_entry_pairs);
            return res;
          }
        ...
        case DW_LNCT_LLVM_source: {
          res = _dwarf_decode_line_string_form(dbg, lntype, lnform, local_length_size, &line_ptr, line_ptr_end,
                                               (char**)&curline->fi_llvm_source, err);
          if (res != DW_DLV_OK) {
            free(filename_entry_pairs);
            return res;
          }
        ...
        case DW_LNCT_timestamp:
          res = _dwarf_decode_line_udata_form(dbg, lntype, lnform, &line_ptr, &curline->fi_time_last_mod, line_ptr_end,
                                              err);
          if (res != DW_DLV_OK) {
            free(filename_entry_pairs);
            return res;
          ...
          break;
        case DW_LNCT_size:
          res = _dwarf_decode_line_udata_form(dbg, lntype, lnform, &line_ptr, &curline->fi_file_length, line_ptr_end,
                                              err);
          if (res != DW_DLV_OK) {
            free(filename_entry_pairs);
            return res;
        ...
        case DW_LNCT_MD5: { /* form DW_FORM_data16 */
          if (filename_entry_pairs[j].up_second != DW_FORM_data16) {
            free(filename_entry_pairs);
            _dwarf_error(dbg, err, DW_DLE_LNCT_MD5_WRONG_FORM);
            return DW_DLV_ERROR;
          }
          res = _dwarf_extract_data16(dbg, line_ptr, line_ptr, line_ptr_end, &curline->fi_md5_value, err);
          if (res != DW_DLV_OK) {
            free(filename_entry_pairs);
            return res;
          }
        ...
        default:
          _dwarf_report_bad_lnct(dbg, lntype, DW_DLE_LINE_NUMBER_HEADER_ERROR, "DW_DLE_LINE_NUMBER_HEADER_ERROR", err);
          free(filename_entry_pairs);
          return DW_DLV_ERROR;
        }
        if (line_ptr > line_ptr_end) {
  ...
  if (version == EXPERIMENTAL_LINE_TABLES_VERSION) {
    ...
    if (subprog_entry_types == NULL) {
      _dwarf_error_string(dbg, err, DW_DLE_ALLOC_FAIL, "DW_DLE_ALLOC_FAIL allocating subprog_entry_types");
      return DW_DLV_ERROR;
    }
    ...
    if (subprog_entry_forms == NULL) {
      free(subprog_entry_types);
      _dwarf_error(dbg, err, DW_DLE_ALLOC_FAIL);
      _dwarf_error_string(dbg, err, DW_DLE_ALLOC_FAIL, "DW_DLE_ALLOC_FAIL allocating subprog_entry_forms");
      return DW_DLV_ERROR;
    }

    for (i = 0; i < subprog_format_count; i++) {

      dres = read_uword_de(&line_ptr, subprog_entry_types + i, dbg, err, line_ptr_end);
      if (dres != DW_DLV_OK) {
        free(subprog_entry_types);
      ...
      dres = read_uword_de(&line_ptr, subprog_entry_forms + i, dbg, err, line_ptr_end);
      if (dres != DW_DLV_OK) {
        free(subprog_entry_types);
        free(subprog_entry_forms);
        return dres;
    ...
    dres = read_uword_de(&line_ptr, &subprogs_count, dbg, err, line_ptr_end);
    if (dres != DW_DLV_OK) {
      free(subprog_entry_types);
      free(subprog_entry_forms);
      return dres;
    ...
    if (subprogs_count > total_length) {
      IssueExpError(dbg, err, "Subprogs Count too large to be real", subprogs_count);
      free(subprog_entry_types);
      free(subprog_entry_forms);
      return DW_DLV_ERROR;
    }
    line_context->lc_subprogs = malloc(sizeof(struct Dwarf_Subprog_Entry_s) * subprogs_count);
    ...
    memset(line_context->lc_subprogs, 0, sizeof(struct Dwarf_Subprog_Entry_s) * subprogs_count);
    for (i = 0; i < subprogs_count; i++) {
      struct Dwarf_Subprog_Entry_s* curline = line_context->lc_subprogs + i;
      if (line_ptr >= line_ptr_end) {
        free(subprog_entry_types);
        free(subprog_entry_forms);
        _dwarf_error_string(dbg, err, DW_DLE_LINE_NUMBER_HEADER_ERROR,
      ...
      for (j = 0; j < subprog_format_count; j++) {
        Dwarf_Unsigned lntype = subprog_entry_types[j];
        Dwarf_Unsigned lnform = subprog_entry_forms[j];
        switch (lntype) {
        case DW_LNCT_GNU_subprogram_name:
          res = _dwarf_decode_line_string_form(dbg, lntype, lnform, local_length_size, &line_ptr, line_ptr_end,
                                               (char**)&curline->ds_subprog_name, err);
          if (res != DW_DLV_OK) {
            free(subprog_entry_types);
            free(subprog_entry_forms);
            return res;
        ...
        case DW_LNCT_GNU_decl_file:
          res =
              _dwarf_decode_line_udata_form(dbg, lntype, lnform, &line_ptr, &curline->ds_decl_file, line_ptr_end, err);
          if (res != DW_DLV_OK) {
            free(subprog_entry_forms);
            free(subprog_entry_types);
            return res;
        ...
        case DW_LNCT_GNU_decl_line:
          res =
              _dwarf_decode_line_udata_form(dbg, lntype, lnform, &line_ptr, &curline->ds_decl_line, line_ptr_end, err);
          if (res != DW_DLV_OK) {
            free(subprog_entry_forms);
            free(subprog_entry_types);
            return res;
  ...
  if (version == EXPERIMENTAL_LINE_TABLES_VERSION) {
    lp_begin = line_context->lc_line_prologue_start + line_context->lc_logicals_table_offset;
  } else {
    lp_begin = line_context->lc_line_prologue_start + line_context->lc_prologue_length;
  }
  if (line_ptr > line_ptr_end) {
    _dwarf_error(dbg, err, DW_DLE_LINE_OFFSET_BAD);
    return DW_DLV_ERROR;
...
static int read_line_table_program(Dwarf_Debug dbg, Dwarf_Small* line_ptr, Dwarf_Small* line_ptr_end,
                                   Dwarf_Small* orig_line_ptr, Dwarf_Small* section_start,
                                   Dwarf_Line_Context line_context, Dwarf_Half address_size,
                                   Dwarf_Bool doaddrs, /* Only true if SGI IRIX rqs calling. */
                                   Dwarf_Bool dolines, Dwarf_Bool is_single_table, Dwarf_Bool is_actuals_table,
                                   Dwarf_Error* error, int* err_count_out) {
  Dwarf_Unsigned i = 0;
  ...
  /*  Initialize the one state machine variable that depends on the
      prefix.  */
  _dwarf_set_line_table_regs_default_values(&regs, line_context->lc_version_number, line_context->lc_default_is_stmt);
  ...
  while (line_ptr < line_ptr_end) {
...
#ifdef PRINTING_DETAILS
    {
      ...
      dwarfstring_append_printf_u(&m9a, " [0x%06" DW_PR_DSx "] ",
    ...
    line_ptr++;
    /* 'type' is the output */
    WHAT_IS_OPCODE(type, opcode, line_context->lc_opcode_base, line_context->lc_opcode_length_table, line_ptr,
    ...
    if (type == LOP_DISCARD) {
...
#ifdef PRINTING_DETAILS
      {
        ...
        dwarfstring_append_printf_i(&m9b, "*** DWARF CHECK: DISCARD standard opcode %d ", opcode);
        dwarfstring_append_printf_i(&m9b, "with %d operands: not understood.", opcnt);
      ...
      for (oc = 0; oc < opcnt; oc++) {
        ...
        (void)utmp2;
        ocres = read_uword_de(&line_ptr, &utmp2, dbg, error, line_ptr_end);
...
#ifdef PRINTING_DETAILS
        {
          ...
          dwarfstring_append_printf_u(&m9e, " %" DW_PR_DUu, utmp2);
          dwarfstring_append_printf_u(&m9e, " (0x%" DW_PR_XZEROS DW_PR_DUx ")", utmp2);
      ...
      if (line_context->lc_maximum_ops_per_instruction < 2) {
        regs.lr_address = regs.lr_address + (operation_advance * line_context->lc_minimum_instruction_length);
      } else {
        regs.lr_address =
            regs.lr_address + (line_context->lc_minimum_instruction_length *
                               ((regs.lr_op_index + operation_advance) / line_context->lc_maximum_ops_per_instruction));
        regs.lr_op_index = (regs.lr_op_index + operation_advance) % line_context->lc_maximum_ops_per_instruction;
      }

      regs.lr_line = regs.lr_line + line_context->lc_line_base + opcode % line_context->lc_line_range;
      if ((Dwarf_Signed)regs.lr_line < 0) {
        ...
        _dwarf_error_string(dbg, error, DW_DLE_LINE_TABLE_LINENO_ERROR, dwarfstring_string(&m));
        dwarfstring_destructor(&m);
        regs.lr_line = 0;
        _dwarf_free_chain_entries(dbg, head_chain, line_count);
        if (curr_line) {
...
#ifdef PRINTING_DETAILS
      {
        ...
        print_line_detail(dbg, dwarfstring_string(&mb), (int)opcode, (unsigned)(line_count + 1), &regs, is_single_table,
                          is_actuals_table);
      ...
      if (dolines) {
        ...
        /* Mark a line record as being DW_LNS_set_address */
        curr_line->li_l_data.li_is_addr_set = is_addr_set;
        is_addr_set = false;
        curr_line->li_address = regs.lr_address;
        curr_line->li_l_data.li_file = (Dwarf_Signed)regs.lr_file;
        curr_line->li_l_data.li_line = (Dwarf_Signed)regs.lr_line;
        curr_line->li_l_data.li_column = (Dwarf_Half)regs.lr_column;
        curr_line->li_l_data.li_is_stmt = regs.lr_is_stmt;
        curr_line->li_l_data.li_basic_block = regs.lr_basic_block;
        curr_line->li_l_data.li_end_sequence = curr_line->li_l_data.li_epilogue_begin = regs.lr_epilogue_begin;
        curr_line->li_l_data.li_prologue_end = regs.lr_prologue_end;
        curr_line->li_l_data.li_isa = regs.lr_isa;
        curr_line->li_l_data.li_discriminator = regs.lr_discriminator;
        curr_line->li_l_data.li_call_context = regs.lr_call_context;
        curr_line->li_l_data.li_subprogram = regs.lr_subprogram;
        curr_line->li_context = line_context;
        curr_line->li_is_actuals_table = is_actuals_table;
        line_count++;

        chain_line = (Dwarf_Chain)_dwarf_get_alloc(dbg, DW_DLA_CHAIN, 1);
        if (chain_line == NULL) {
          if (curr_line) {
            dwarf_dealloc(dbg, curr_line, DW_DLA_LINE);
          }
...
#ifdef PRINTING_DETAILS
        print_line_detail(dbg, "DW_LNS_copy", opcode, (unsigned int)line_count + 1, &regs, is_single_table,
                          is_actuals_table);
#endif /* PRINTING_DETAILS */
        if (dolines) {
          curr_line = (Dwarf_Line)_dwarf_get_alloc(dbg, DW_DLA_LINE, 1);
      ...
      case DW_LNS_advance_pc: {
        ...
        advres = read_uword_de(&line_ptr, &utmp2, dbg, error, line_ptr_end);
...
#ifdef PRINTING_DETAILS
        dwarfstring_constructor(&mb);
        dwarfstring_append_printf_i(&mb, "DW_LNS_advance_pc val %" DW_PR_DSd, utmp2);
        dwarfstring_append_printf_u(&mb, " 0x%" DW_PR_XZEROS DW_PR_DUx "\n", utmp2);
        _dwarf_printf(dbg, dwarfstring_string(&mb));
        dwarfstring_destructor(&mb);
#endif /* PRINTING_DETAILS */
        leb128_num = utmp2;
        regs.lr_address = regs.lr_address + line_context->lc_minimum_instruction_length * leb128_num;
      } break;
      case DW_LNS_advance_line: {
        Dwarf_Signed stmp = 0;
        int alres = 0;

        alres = read_sword_de(&line_ptr, &stmp, dbg, error, line_ptr_end);
...
#ifdef PRINTING_DETAILS
        dwarfstring_constructor(&mb);
        dwarfstring_append_printf_i(&mb, "DW_LNS_advance_line val %" DW_PR_DSd, advance_line);
      ...
      case DW_LNS_set_file: {
        ...
        sfres = read_uword_de(&line_ptr, &utmp2, dbg, error, line_ptr_end);
        ...
        }
        {
          ...
          if (fno < 0) {
            ...
            _dwarf_error_string(dbg, error, DW_DLE_LINE_INDEX_WRONG,
...
#ifdef PRINTING_DETAILS
        dwarfstring_constructor(&mb);
        dwarfstring_append_printf_i(&mb, "DW_LNS_set_file  %ld\n", regs.lr_file);
        _dwarf_printf(dbg, dwarfstring_string(&mb));
        dwarfstring_destructor(&mb);
#endif /* PRINTING_DETAILS */
      } break;
      case DW_LNS_set_column: {
        ...
        scres = read_uword_de(&line_ptr, &utmp2, dbg, error, line_ptr_end);
        ...
        dwarfstring_append_printf_i(&mb, "DW_LNS_set_column val %" DW_PR_DSd, regs.lr_column);
        dwarfstring_append_printf_u(&mb, " 0x%" DW_PR_XZEROS DW_PR_DSx "\n", regs.lr_column);
        _dwarf_printf(dbg, dwarfstring_string(&mb));
        dwarfstring_destructor(&mb);
#endif /* PRINTING_DETAILS */
...
#ifdef PRINTING_DETAILS
        _dwarf_printf(dbg, "DW_LNS_set_basic_block\n");
#endif /* PRINTING_DETAILS */
      } break;
      ...
      case DW_LNS_const_add_pc: {
        ...
        if (line_context->lc_maximum_ops_per_instruction < 2) {
          Dwarf_Unsigned operation_advance = (opcode / line_context->lc_line_range);
          regs.lr_address = regs.lr_address + line_context->lc_minimum_instruction_length * operation_advance;
        } else {
          Dwarf_Unsigned operation_advance = (opcode / line_context->lc_line_range);
...
#ifdef PRINTING_DETAILS
        dwarfstring_constructor(&mb);
        dwarfstring_append_printf_u(&mb, "DW_LNS_const_add_pc new address 0x%" DW_PR_XZEROS DW_PR_DSx "\n",
                                    regs.lr_address);
        _dwarf_printf(dbg, dwarfstring_string(&mb));
        dwarfstring_destructor(&mb);
#endif /* PRINTING_DETAILS */
      ...
      case DW_LNS_fixed_advance_pc: {
        ...
        apres = _dwarf_read_unaligned_ck_wrapper(dbg, &fpc, line_ptr, DWARF_HALF_SIZE, line_ptr_end, error);
        ...
        if (line_ptr > line_ptr_end) {
          dwarfstring g;
          /*  ptrdiff_t is generated but not named */
          Dwarf_Unsigned localoff = (line_ptr >= section_start) ? (line_ptr - section_start) : 0xfffffff;
          ...
          dwarfstring_append_printf_u(&g,
                                      ...
                                      "offset. 0x%x .",
                                      localoff);
          _dwarf_error_string(dbg, error, DW_DLE_LINE_TABLE_BAD, dwarfstring_string(&g));
...
#ifdef PRINTING_DETAILS
        dwarfstring_constructor(&mb);
        dwarfstring_append_printf_i(&mb, "DW_LNS_fixed_advance_pc val %" DW_PR_DSd, fixed_advance_pc);
        dwarfstring_append_printf_u(&mb, " 0x%" DW_PR_XZEROS DW_PR_DSx, fixed_advance_pc);
        dwarfstring_append_printf_u(&mb, " new address 0x%" DW_PR_XZEROS DW_PR_DSx "\n", regs.lr_address);
        _dwarf_printf(dbg, dwarfstring_string(&mb));
        dwarfstring_destructor(&mb);
#endif /* PRINTING_DETAILS */
      } break;
...
#ifdef PRINTING_DETAILS
        _dwarf_printf(dbg, "DW_LNS_set_prologue_end set true.\n");
#endif /* PRINTING_DETAILS */
      } break;
      ...
      case DW_LNS_set_isa: {
        ...
        sires = read_uword_de(&line_ptr, &utmp2, dbg, error, line_ptr_end);
...
#ifdef PRINTING_DETAILS
        dwarfstring_constructor(&mb);
        dwarfstring_append_printf_u(&mb, "DW_LNS_set_isa new value 0x%" DW_PR_XZEROS DW_PR_DUx ".\n", utmp2);
        _dwarf_printf(dbg, dwarfstring_string(&mb));
        dwarfstring_destructor(&mb);
#endif /* PRINTING_DETAILS */
      ...
      case DW_LNS_set_subprogram:
        ...
        if (is_actuals_table) {
          ...
          atres = read_sword_de(&line_ptr, &stmp, dbg, error, line_ptr_end);
          ...
          if (regs.lr_line >= 1 && regs.lr_line - 1 < logicals_count) {
            regs.lr_address = logicals[regs.lr_line - 1]->li_address;
            regs.lr_op_index = 0;
            ...
            dwarfstring_append_printf_i(&mb,
                                        ...
                                        "%" DW_PR_DSd,
                                        stmp);
            dwarfstring_append_printf_u(&mb, " 0x%" DW_PR_XZEROS DW_PR_DSx, stmp);
          ...
          spres = read_uword_de(&line_ptr, &utmp2, dbg, error, line_ptr_end);
      ...
      case DW_LNS_inlined_call: {
        ...
        icres = read_sword_de(&line_ptr, &stmp, dbg, error, line_ptr_end);
        ...
        icres = read_uword_de(&line_ptr, &ilcuw, dbg, error, line_ptr_end);
        ...
        dwarfstring_append_printf_i(&mb,
                                    ...
                                    stmp);
        dwarfstring_append_printf_u(&mb, " (0x%" DW_PR_XZEROS DW_PR_DSx "),", stmp);
        dwarfstring_append_printf_i(&mb, "%" DW_PR_DSd, regs.lr_subprogram);
        dwarfstring_append_printf_u(&mb, " (0x%" DW_PR_XZEROS DW_PR_DSx ")", regs.lr_subprogram);
        dwarfstring_append_printf_i(&mb,
                                    "  callcontext="
                                    "%" DW_PR_DSd,
      ...
      case DW_LNS_pop_context: {
        ...
        if (logical_num > 0 && logical_num <= line_count) {
          ...
          regs.lr_file = logical_line->li_l_data.li_file;
          regs.lr_line = logical_line->li_l_data.li_line;
          regs.lr_column = logical_line->li_l_data.li_column;
          regs.lr_discriminator = logical_line->li_l_data.li_discriminator;
          regs.lr_is_stmt = logical_line->li_l_data.li_is_stmt;
          regs.lr_call_context = logical_line->li_l_data.li_call_context;
          regs.lr_subprogram = logical_line->li_l_data.li_subprogram;
#ifdef PRINTING_DETAILS
          {
            dwarfstring pcon;
            ...
            dwarfstring_append_printf_u(&pcon,
                                        ...
                                        "%" DW_PR_DUu,
                                        logical_num);
            dwarfstring_append_printf_u(&pcon, " (0x%" DW_PR_XZEROS DW_PR_DUx ")\n", logical_num);
            _dwarf_printf(dbg, dwarfstring_string(&pcon));
      ...
      leres = read_uword_de(&line_ptr, &utmp3, dbg, error, line_ptr_end);
      ...
      if (line_ptr >= line_ptr_end) {
        dwarfstring g;
        /*  ptrdiff_t is generated but not named */
        Dwarf_Unsigned localoffset = (line_ptr >= section_start) ? (line_ptr - section_start) : 0;
      ...
      case DW_LNE_end_sequence: {
...
#ifdef PRINTING_DETAILS
          print_line_detail(dbg, "DW_LNE_end_sequence extended", (int)ext_opcode, (unsigned int)line_count + 1, &regs,
        ...
        _dwarf_set_line_table_regs_default_values(&regs, line_context->lc_version_number,
      ...
      case DW_LNE_set_address: {
        ...
        sares = _dwarf_read_unaligned_ck_wrapper(dbg, &regs.lr_address, line_ptr, address_size, line_ptr_end, error);
...
#ifdef PRINTING_DETAILS
        {
          ...
          dwarfstring_append_printf_u(&sadd, "DW_LNE_set_address address 0x%" DW_PR_XZEROS DW_PR_DUx "\n",
                                      regs.lr_address);
        ...
        if (doaddrs) {
          ...
          /*  ptrdiff_t is generated but not named */
          curr_line->li_offset = line_ptr - dbg->de_debug_line.dss_data;
#endif /* __sgi */
          line_count++;
          chain_line = (Dwarf_Chain)_dwarf_get_alloc(dbg, DW_DLA_CHAIN, 1);
          if (chain_line == NULL) {
      ...
      case DW_LNE_define_file:
        if (dolines) {
          ...
          cur_file_entry = (Dwarf_File_Entry)malloc(sizeof(struct Dwarf_File_Entry_s));
          ...
          dlres =
              _dwarf_check_string_valid(dbg, line_ptr, line_ptr, line_ptr_end, DW_DLE_DEFINE_FILE_STRING_BAD, error);
          if (dlres != DW_DLV_OK) {
            _dwarf_free_chain_entries(dbg, head_chain, line_count);
            if (curr_line) {
              dwarf_dealloc(dbg, curr_line, DW_DLA_LINE);
          ...
          line_ptr = line_ptr + strlen((char*)line_ptr) + 1;
          dlres = read_uword_de(&line_ptr, &value, dbg, error, line_ptr_end);
...
#ifdef PRINTING_DETAILS
          {
            ...
            dwarfstring_append_printf_s(&m9c, "DW_LNE_define_file %s \n", (char*)cur_file_entry->fi_file_name);
            dwarfstring_append_printf_i(&m9c, "    dir index %d\n", (int)cur_file_entry->fi_dir_index);

            {
              time_t tt3 = (time_t)cur_file_entry->fi_time_last_mod;

              /* ctime supplies newline */
              dwarfstring_append_printf_u(&m9c, "    last time 0x%x ", (Dwarf_Unsigned)tt3);
              dwarfstring_append_printf_s(&m9c, "%s", ctime(&tt3));
            }
            dwarfstring_append_printf_i(&m9c, "    file length %ld ", cur_file_entry->fi_file_length);
            dwarfstring_append_printf_u(&m9c, "0x%lx\n", cur_file_entry->fi_file_length);
      ...
      case DW_LNE_set_discriminator: {
        ...
        sdres = read_uword_de(&line_ptr, &utmp2, dbg, error, line_ptr_end);
...
#ifdef PRINTING_DETAILS
        {
          ...
          dwarfstring_append_printf_u(&mk, "DW_LNE_set_discriminator 0x%" DW_PR_XZEROS DW_PR_DUx "\n", utmp2);
      ...
      default: {
        ...
        /*  ptrdiff_t is generated but not named */
        Dwarf_Unsigned space_left = (line_ptr <= line_ptr_end) ? (line_ptr_end - line_ptr) : 0xfffffff;

        /*  By catching this here instead of PRINTING_DETAILS
            we avoid reading off of our data of interest*/
        if (instr_length < 1 || space_left < remaining_bytes || remaining_bytes > DW_LNE_LEN_MAX) {
          dwarfstring g;
          /*  ptrdiff_t is generated but not named */
          Dwarf_Unsigned localoff = (line_ptr >= section_start) ? (line_ptr - section_start) : 0xfffffff;
...
#ifdef PRINTING_DETAILS
        {
          ...
          dwarfstring_append_printf_u(&m9d, "DW_LNE extended op 0x%x ", ext_opcode);
          dwarfstring_append_printf_u(&m9d, "Bytecount: %" DW_PR_DUu, (Dwarf_Unsigned)instr_length);
          if (remaining_bytes > 0) {
            /*  If remaining bytes > distance to end
                we will have an error. */
            dwarfstring_append(&m9d, " linedata: 0x");
            while (remaining_bytes > 0) {
              dwarfstring_append_printf_u(&m9d, "%02x", (unsigned char)(*(line_ptr)));
              line_ptr++;
#if 0
  ...
  block_line = (Dwarf_Line*)_dwarf_get_alloc(dbg, DW_DLA_LIST, line_count);
  if (block_line == NULL) {
    ...
    if (is_single_table) {
      if (!line_count) {
        dwarfstring_append_printf_u(
            &mc, " Line table is present (offset 0x%" DW_PR_XZEROS DW_PR_DUx ") but no lines present\n",
            line_context->lc_section_offset);
      }
    } else if (is_actuals_table) {
      if (!line_count) {
        dwarfstring_append_printf_u(
            &mc, " Line table present (offset 0x%" DW_PR_XZEROS DW_PR_DUx ") but no actuals lines present\n",
            line_context->lc_section_offset);
      }
    } else {
      if (!line_count) {
        dwarfstring_append_printf_u(
            &mc, " Line table present (offset 0x%" DW_PR_XZEROS DW_PR_DUx ") but no logicals lines present\n",
            line_context->lc_section_offset);
      }
    }
